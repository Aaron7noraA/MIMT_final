...
class Pframe(CompressesModel):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                rec_frame, likelihoods, _, flow_hat, mc_frame, _, _, BDQ, mc_hat = self(align.align(ref_frame),
    ...
    def test_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            ...
            if frame_idx != 0:
                ...
                ######## Choose reference frame from all previously reconstructed frames #######
                #CHOOSE_BY = "PSNR"
                CHOOSE_BY = "loss"
                #CHOOSE_BY = "RD"
                min_loss = np.inf
                ...
                # reverse the buffer so that {frame_idx-1}-th frame will be the first to be calculated
                if CHOOSE_BY == 'RD' or CHOOSE_BY == 'PSNR': 
                    ref_frame_buffer = ref_frame_buffer[::-1]
                for ref_frame_idx, ref_frame in enumerate(ref_frame_buffer):
                    rec_frame, likelihoods, _, flow_hat, mc_frame, _, _, BDQ, mc_hat = self(
                    ...
                    mse = self.criterion(rec_frame, batch[:, frame_idx]).mean().item()
                    psnr = mse2psnr(mse)
                    testing_loss = self.args.lmda * mse + rate
                

                    if CHOOSE_BY == 'RD':
                        _ref_frame_idx = len(ref_frame_buffer) - 1 - ref_frame_idx # index in increasing frame order
                        if psnr > max_psnr and rate < min_rate:
                            max_psnr = psnr
                            min_rate = rate
                            min_ref_frame_idx = _ref_frame_idx
                    elif CHOOSE_BY == 'loss':
                        _ref_frame_idx = ref_frame_idx
                    ...
                    #print(_ref_frame_idx, " - ", "PSNR = ", psnr, "; Rate = ", rate, "; Loss =", testing_loss)
                    #print('my =', estimate_bpp(likelihoods[0]).item(), 'mz =', estimate_bpp(likelihoods[1]).item())
                    #print('ry =', estimate_bpp(likelihoods[2]).item(), 'rz =', estimate_bpp(likelihoods[3]).item())
                    #print('========================================')
                if CHOOSE_BY == 'RD' or CHOOSE_BY == 'PSNR': # reverse the buffer back
                    ref_frame_buffer = ref_frame_buffer[::-1]
                #print("min_ref_frame_idx: ", min_ref_frame_idx)
                #print(f"-----------------------------")
                ################################## End ########################################

                # Inference with the chosen reference frame again
                ref_frame = ref_frame_buffer[min_ref_frame_idx]
                ...
                log_list.append({'PSNR': psnr, 'Rate': rate, 'MC-PSNR': mc_psnr,
                                 ...
                                 'MCerr-PSNR': mc_err_psnr, 'BDQ-PSNR': BDQ_psnr, 'ref_frame_idx': min_ref_frame_idx})
            ...
            ref_frame_buffer.append(ref_frame)
    ...
    def setup(self, stage):
        ...
        if stage == 'fit':
            ...
            self.test_dataset = VideoTestDataIframe(dataset_root + "TestVideo", self.args.lmda, sequence=('B'))
