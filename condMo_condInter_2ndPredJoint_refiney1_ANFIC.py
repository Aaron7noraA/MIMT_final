...
class Pframe(CompressesModel):
    def __init__(self, args, mo_coder, cond_mo_coder, res_coder):
        ...
        self.feature_extractors_y = nn.ModuleList([ResidualBlock(3, 32), DownsampleBlock(32, 64), DownsampleBlock(64, 96)])

        self.MCNet = GridNet([6, 64, 128, 192], [32, 64, 96], 6, 3)
        self.MCNet_y = GridNet([6, 64, 128, 192], [32, 64, 96], 6, 3)
        ...
        self.MCNet_y.load_state_dict(new_ckpt, strict=True)
    ...
    def motion_compensation(self, ref_frame, flow, feature_extractors, refine_net):
        ...
        for i, feature_extractor in enumerate(feature_extractors):
            ...
            feats1.append(self.Resampler(feat, nn.functional.interpolate(flow, scale_factor=2**(-i), mode='bilinear', align_corners=True)))
            feats2.append(feat)

        feats = [torch.cat([feat1, feat2], axis=1)  for feat1, feat2 in zip(feats1, feats2)]
        mc_frame, _ = refine_net(feats)
    ...
    def motion_forward(self, ref_frame, coding_frame, visual=False, visual_prefix='', predict=False):
        if predict:
            ...
            mc_frame, warped_frame = self.motion_compensation(ref_frame, flow_hat, self.feature_extractors, self.MCNet)
    ...
    def forward(self, ref_frame, coding_frame, prev_y1=None, p_order=0, visual=False, visual_prefix='', predict=False):
        ...
        refined_y1, _ = self.motion_compensation(prev_y1, m_info['flow_hat'], self.MCNet_y)
        refined_y1 = torch.cat([mc, refined_y1], dim=1)

        reconstructed, likelihood_r, x_2, _, _, BDQ, decoded = self.Residual(coding_frame, output=None,
                                                                        cond_coupling_input=[mc, refined_y1], pred_prior_input=mc,
            ...
            for frame_idx in range(1, 6):
                ...
                with torch.set_grad_enabled(epoch >= phase['trainRes_2frames']): 
                    if frame_idx == 1:
                        ...
                        prev_y1 = decoded[0]
                        prev_y1, _ = self.motion_compensation(prev_y1, data['flow_hat'], self.feature_extractors_y, self.MCNet_y)

                y1_cond = torch.cat([mc, prev_y1], dim=1)
                reconstructed, likelihood_r, x_2, _, _, _, decoded = self.Residual(coding_frame, output=None,
                                                                        cond_coupling_input=[mc, y1_cond], pred_prior_input=mc,)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                y1_cond = torch.cat([mc_frame, prev_y1], dim=1)

                rec_frame, likelihood_r, mc_hat, _, _, BDQ, decoded = self.Residual(align.align(coding_frame), output=None,
                                                                        cond_coupling_input=[mc_frame, y1_cond], pred_prior_input=mc_frame,)
                ...
                prev_y1 = align.resume(prev_y1).clamp(0, 1)
                ...
                if frame_idx == 5:
                    ...
                    upload_img(prev_y1.cpu().numpy()[0], f'{seq_name}_{epoch}_warped_y1.png', grid=False)
    ...
    def test_step(self, batch, batch_idx):
        metrics_name = ['PSNR', 'Rate', 'Mo_Rate', 'MC-PSNR', 'BDQ-PSNR', 'QE-PSNR', 'p1-PSNR', 'p1-BDQ-PSNR']
        metrics = {}
        ...
        for frame_idx in range(gop_size):
            ...
            TO_VISUALIZE = frame_id_start == 1 and seq_name in ['Cactus', 'HoneyBee', 'Jockey']
            ...
            if frame_idx != 0:
                ...
                y1_cond = torch.cat([mc_frame, prev_y1], dim=1)
                rec_frame, likelihood_r, mc_hat, _, _, BDQ, decoded = self.Residual(align.align(coding_frame), output=None,
                                                                           cond_coupling_input=[mc_frame, y1_cond], 
                                                                           pred_prior_input=mc_frame,
                                                                           visual=visual, figname=figname)
                ...
                if TO_VISUALIZE:
                    ...
                    save_image(prev_y1[0], self.args.save_dir + f'/{seq_name}/mc_frame/'
                                                                  f'prev_y1_{int(frame_id_start + frame_idx)}.png')
...
if __name__ == '__main__':
    ...
    if args.restore == 'resume' or args.restore == 'finetune':
        ...
        gridnet_ckpt = torch.load(os.path.join(save_root, "CANFVC_Plus", "e6a3c29991d144dbbe71e4dc717170ac", "checkpoints", "epoch=40.ckpt"),
                                map_location=(lambda storage, loc: storage))
        coder_ckpt = torch.load(os.path.join(os.getenv('LOG', './'), f"ANFIC/ANFHyperPriorCoder_{ANFIC_code}/model.ckpt"),
                                map_location=(lambda storage, loc: storage))['coder']
        from collections import OrderedDict                  
        ...
        for k, v in checkpoint['state_dict'].items():
            new_ckpt[k] = v
        for k, v in gridnet_ckpt['state_dict'].items():
            if k.split('.')[0] == 'feature_extractors':
                new_ckpt[k] = v
                key = '.'.join(['feature_extractors_y'] + k.split('.')[1:])
                new_ckpt[key] = v
        ...
        #gridnet_ckpt = torch.load(os.path.join(save_root, "CANFVC_Plus", "e6a3c29991d144dbbe71e4dc717170ac", "checkpoints", "epoch=40.ckpt"),
        #                        map_location=(lambda storage, loc: storage))
        #pframe_ckpt = torch.load(os.path.join(save_root, "CANFVC_Plus", "e3d451b979d04081826ff7575abd68f3", "checkpoints", "epoch=25.ckpt"),
        #                        map_location=(lambda storage, loc: storage))

        #from collections import OrderedDict
        #new_ckpt = OrderedDict()

        #for k, v in pframe_ckpt['state_dict'].items():
        #    if k.split('.')[0] != 'MCNet' and k.split('.')[0] != 'Residual':
        #        new_ckpt[k] = v
        #    elif k.split('.')[0] == 'Residual':
        #        if not ((k.split('.')[1] == 'analysis1' and k.split('.')[2] == '0') or (k.split('.')[1] == 'synthesis1' and k.split('.')[2] == 'generalized_sum')):
        #            new_ckpt[k] = v
        #for k, v in gridnet_ckpt['state_dict'].items():
        #    if k.split('.')[0] == 'MCNet':
        #        new_ckpt[k] = v
        #        key = '.'.join(['MCNet_y'] + k.split('.')[1:])
        #        new_ckpt[key] = v
        #    elif k.split('.')[0] == 'feature_extractors':
        #        new_ckpt[k] = v
