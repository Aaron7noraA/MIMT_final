...
class Pframe(CompressesModel):
    ...
    def forward(self, ref_frame, coding_frame, prev_y1=None, p_order=0, visual=False, visual_prefix='', predict=False):
        ...
        reconstructed, likelihood_r, x_2, _, _, BDQ, decoded = self.Residual(coding_frame, output=None,
                                                                        cond_coupling_input=[mc, prev_y1], pred_prior_input=None,
        ...
        if epoch < phase['trainMV']:
            ...
            for frame_idx in range(1, 7):
                ...
                if self.args.ssim:
                    ...
                    mc_error = nn.MSELoss(reduction='none')(x_2, mc)
                else:
                    mc_error = nn.MSELoss(reduction='none')(x_2, mc)
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                rec_frame, likelihood_r, mc_hat, _, _, BDQ, decoded = self.Residual(align.align(coding_frame), output=None,
                                                                        cond_coupling_input=[mc_frame, prev_y1], pred_prior_input=None)
    ...
    def test_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            ...
            TO_VISUALIZE = False and frame_id_start == 1 and frame_idx < 8 and seq_name in ['Bosphorus', 'Kimono1', 'HoneyBee', 'Jockey', 'ReadySteadyGo', 'ShakeNDry']
            ...
            if frame_idx != 0:
                ...
                if TO_VISUALIZE:
                    ...
                    save_image(cur_y1[0], self.args.save_dir + f'/{seq_name}/mc_frame/'
                                                                  f'cur_y1_{int(frame_id_start + frame_idx)}.png')
                    save_image(0.5 + cur_y1[0] - prev_y1[0], self.args.save_dir + f'/{seq_name}/mc_frame/'
                                                                  f'y1_diff_{int(frame_id_start + frame_idx)}.png')
...
if __name__ == '__main__':
    ...
    if args.restore == 'resume' or args.restore == 'finetune':
        ...
        trainer.current_epoch = phase['trainRes_2frames']
