...
from models import Refinement, MultiFrameMergeNet
...
class Pframe(CompressesModel):
    def __init__(self, args, mo_coder, cond_mo_coder, res_coder):
        ...
        self.MFF = MultiFrameMergeNet(6, 2)
    ...
    def forward(self, intra_frame, previous_frame, coding_frame, p_order=0, visual=False, visual_prefix=''):
        ...
        if "SepPA" in self.args.residual_coder_conf:
            likelihood_ry, likelihood_rz = likelihood_r
            likelihood_ry, likelihood_ry_nocontext = likelihood_ry
            likelihood_r = (likelihood_ry, likelihood_rz)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            if epoch <= phase['trainRes_2frames']:
                frozen_modules = [
                    self.Motion, self.MWNet, self.CondMotion, self.frame_synthesis, *self.feature_extractors,
                    #self.Residual.analysis0.main_transform,
                    #self.Residual.synthesis0.main_transform,
                    #self.Residual.analysis1.main_transform,
                    #self.Residual.synthesis1.main_transform,
                ]
                reactivate_modules = []
            else:
                frozen_modules = [self.Motion, self.MWNet]
                reactivate_modules = [
                    self.CondMotion, self.frame_synthesis, *self.feature_extractors,
                    #self.Residual.analysis0.main_transform,
                    #self.Residual.synthesis0.main_transform,
                    #self.Residual.analysis1.main_transform,
                    #self.Residual.synthesis1.main_transform,
            ...
            if self.args.restore == 'finetune':
                frozen_modules = []
                reactivate_modules = []
            elif self.args.restore == 'custom_finetune':
                ...
                if "SepPA" in self.args.residual_coder_conf:
                    reactivate_modules = [self.Residual.conditional_bottleneck.PA, self.Residual.conditional_bottleneck.score_est_net]
                else:
                    reactivate_modules = [self.Residual.conditional_bottleneck]
            else:
            ...
            for frame_idx in range(1, 5):
                ...
                if "SepPA" in self.args.residual_coder_conf:
                    ...
                    likelihood_ry, likelihood_ry_nocontext = likelihood_ry, likelihood_ry

                rate_m = trc.estimate_bpp(likelihood_m, input=coding_frame)
                rate_r = trc.estimate_bpp((likelihood_ry, likelihood_rz), input=coding_frame)
                rate_r_nocontext = trc.estimate_bpp((likelihood_ry_nocontext, likelihood_rz), input=coding_frame)
                rate = rate_m + rate_r_nocontext #+ rate_r - rate_r.clone().detach() 
                ...
                rate_r_nocontext_list.append(rate_r_nocontext.mean())
            ...
            rate_r_nocontext = torch.mean(torch.tensor(rate_r_nocontext_list))
            ...
            logs = {
                    ...
                    'train/rate_r_nocontext': rate_r_nocontext.item(), 
    ...
    def test_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            ...
            TO_VISUALIZE = False and frame_id_start == 1 and frame_idx < 8# and seq_name in ['Jockey']
...
if __name__ == '__main__':
    ...
    if args.restore == 'resume':
        ...
        coder_ckpt = torch.load(os.path.join(os.getenv('LOG', './'), f"ANFIC/ANFHyperPriorCoder_{ANFIC_code}/model.ckpt"),
                                map_location=(lambda storage, loc: storage))['coder']
        #pretrain_ckpt = torch.load(os.path.join(save_root, project_name, "ec87005e27ce4b92af9a629e8c57b694", "checkpoints", "epoch=33.ckpt"),
        #                        map_location=(lambda storage, loc: storage))
        ...
        #    if k.split('.')[0] == 'Residual' and k.split('.')[1] == 'conditional_bottleneck':
        #        new_ckpt[k] = v
        for k, v in checkpoint['state_dict'].items():
            if k.split('.')[0] == 'Residual' and k.split('.')[1] == 'PA' :
                key = k.replace('PA', 'conditional_bottleneck.PA0')
                new_ckpt[key] = v
            else:
                new_ckpt[k] = v
            #new_ckpt[k] = v
