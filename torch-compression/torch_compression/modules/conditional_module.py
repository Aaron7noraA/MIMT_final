...
def gen_random_condition(lmdas, batch_size, shuffle=False, device='cpu'):
    ...
    if batch_size >= len(lmdas):
        rands = np.exp(np.random.uniform(max(np.log(np.min(lmdas))-0.5, 0), np.log(
            np.max(lmdas))+0.5, batch_size-len(lmdas)))
        lmdas = np.concatenate([np.array(lmdas), rands])
    else:
        lmdas = np.exp(np.random.uniform(max(np.log(np.min(lmdas))-0.5, 0), np.log(
            np.max(lmdas))+0.5, batch_size))
