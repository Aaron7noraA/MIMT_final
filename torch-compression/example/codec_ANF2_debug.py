...
try:
    ...
    from util.loss import PSNR, MS_SSIM, huber_loss
except:
    from .util.auto_helper import cptree, ffilter, mkdir
    from .util.datasets import MSCOCO, CLICTrain, CustomData, Kodak, Vimeo90K
    from .util.log_manage import (AvgMeter, dump_args, gen_log_folder_name,
...
def train(args):
    ...
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optim, factor=0.9, patience=10, verbose=True)  # OPTIM
    ...
    def evaluate(epoch, eval_lmda=None, is_resume=False):
        ...
        for idx, eval_imgs in enumerate(validate_dataloader):
            ...
            eval_imgs_tilde, eval_likelihoods, Y_error, jac, _ = coder(
                eval_imgs, visual=-2 if is_resume else (-1 if epoch == 0 else epoch), figname=os.path.join(args.checkpoint_dir, "kodak_%02d" % (idx+1)))
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for iter, (imgs, path) in enumerate(pbar):
            imgs = imgs.to(DEVICE)
            ...
            if iter_count > 1000:
                ...
                }, os.path.join(args.checkpoint_dir, f"model_{suffix}.ckpt"))
            with open(args.checkpoint_dir+"/path_%d.txt" % iter_count, "w") as fp:
                for pa in path:
                    fp.write(pa+"\n")

            if args.parallel:
                imgs_tilde, likelihoods, Y_error, jac, y_err = model_forward(
            ...
            ), os.path.join(args.checkpoint_dir, f"gradflow_{suffix}.png"))
            if args.clip_max_norm > 0:
                nn.utils.clip_grad_norm_(main_parameters, args.clip_max_norm)
            trc.util.vision.plot_grad_flow(coder.named_main_parameters(
            ), os.path.join(args.checkpoint_dir, f"gradflow_{suffix}_clip.png"))
            if args.check_grad:
                trc.AugmentedNormalizedFlows.dump_grad(
                    os.path.join(grad_log, f"grad_log_{iter_count}.txt"))
            ...
            if iter_count > 1000:
                save_image(torch.cat(
                    [imgs, imgs_tilde]), args.checkpoint_dir+f"/imgs_{suffix}.png", nrow=args.batch_size)

            loss_mean = loss_queue.mean()
            if iter_count > 2000 and rd_loss.item() > loss_mean * 2:
                # save_image(torch.cat(
                ...
                #                   map_location=DEVICE)
                # coder.load_state_dict(ckpt['coder'])
                print("ERROR at", iter_count, "cur",
                      rd_loss.item(), "mean", loss_mean)
                error_count += 1
            else:
        ...
        if rd_loss.gt(1e3).any():
            torch.save({
                'epoch': epoch,
                'coder': coder.state_dict(),
                'optim': optim.state_dict()
            }, os.path.join(args.checkpoint_dir, "model_error.ckpt"))
