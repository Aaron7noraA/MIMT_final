...
from torch_compression.modules.conditional_module import (
    ConditionalLayer, conditional_warping, gen_discrete_condition,
    set_condition)
...
def train(args):
    ...
    def evaluate(epoch, eval_lmda, is_resume=False):
        ...
        # coder.train()
        cond, lmda = gen_discrete_condition(
            args.lmda, len(args.lmda), device=DEVICE)
        nearest_lmda = lmda.new_tensor(eval_lmda).view(
            1, 1).sub(lmda).abs().min(0)[1]
        cond, lmda = cond[nearest_lmda], lmda[nearest_lmda]
    ...
    cond_warping = partial(conditional_warping,
                           conditions=len(args.lmda), ver=1)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for iter, imgs in enumerate(pbar):
            imgs = imgs.to(DEVICE)
            cond, lmda = gen_discrete_condition(
                args.lmda, args.batch_size, device=DEVICE)
            # print(lmda)
            set_condition(coder, cond)
            lmda = lmda.flatten()
