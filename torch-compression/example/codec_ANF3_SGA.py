...
def train(args):
    ...
    coder = trc.hub.AugmentedNormalizedFlowHyperPriorCoder(
        ...
        init_code=args.init_code, use_code=not args.disable_code, dec_add=args.alt_mode != "None", use_attn=args.use_attn, use_QE=args.use_QE,
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.quant_mode == "SGA":
            for m in coder.modules():
                if isinstance(m, trc.StochasticGumbelAnnealing):
                    m.step()
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        help="Use DequantizationModule between layers.")
    parser.add_argument(
