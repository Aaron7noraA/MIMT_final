...
from torch_compression.modules.conditional_module import (gen_condition,
                                                          set_condition)
...
def train(args):
    ...
    coder = trc.hub.AugmentedNormalizedFlowHyperPriorCoder2(
        ...
        init_code=args.init_code, use_code=not args.disable_code, dec_add=args.alt_mode != "None", use_attn=args.use_attn, input_norm=args.input_norm,
        use_syntax=args.use_syntax, use_AQ=args.use_AQ, syntax_prior=args.syntax_prior, 
        use_conditionalconv=len(args.lmda) > 1, ch_wise=args.ch_wise,
    ...
    def evaluate(epoch, eval_lmda, is_resume=False):
        ...
        # coder.train()
        lmda = gen_condition(eval_lmda, 1, device=DEVICE)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for iter, imgs in enumerate(pbar):
            imgs = imgs.to(DEVICE)
            lmda = gen_condition(args.lmda, args.batch_size, device=DEVICE)
            # print(lmda)
            set_condition(coder, lmda)
            lmda = lmda.flatten()
