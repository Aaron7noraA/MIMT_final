...
def train(args):
    ...
    coder = trc.hub.GoogleIDFPriorCoder(
        args.num_filters, args.num_features, args.simplify_gdn,
        num_layers=args.num_layers, num_flows=args.num_flows, factor=args.factor, factor_out=args.factor_out,
        archi=trc.IntegerDescreteFlows.BackBone(
            args.num_IDF_filters, nn_type=args.nn_type),
    ...
    def evaluate(epoch, eval_lmda=None, is_resume=False):
        ...
        for idx, eval_imgs in enumerate(validate_dataloader):
            ...
            eval_imgs_tilde, eval_likelihoods = coder(
                eval_imgs, break_layer=args.break_layer,
                counting=epoch % 3 == 0, figname=os.path.join(args.checkpoint_dir, "kodak_%02d" % (idx+1)))
            rates = [trc.estimate_bpp(
                eval_likelihood, input=eval_imgs) for eval_likelihood in eval_likelihoods]
            eval_rate = sum(rates)
    ...
    if args.resume:
        ...
        if args.reload_AE:
            weights = {}
            for n, p in ckpt['coder'].items():
                if n.split(".")[0] in ['analysis', 'synthesis']:
                    print("Reload:", n)
                    weights[n] = p

            coder.load_state_dict(weights, strict=False)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for imgs in pbar:
            ...
            optim.zero_grad()
            if args.parallel:
                imgs_tilde, likelihoods = model_forward(
                    imgs, module_kwargs=dict(reload_AE=args.reload_AE, break_layer=args.break_layer))
            else:
                imgs_tilde, likelihoods = model_forward(
                    imgs, reload_AE=args.reload_AE, break_layer=args.break_layer)

            rate = trc.estimate_bpp(likelihoods, input=imgs)

            if args.reload_AE:
                distortion = torch.zeros_like(rate)
                rd_loss = rate.mean()
            ...
            pbar.set_description("Epoch: {}/{}: dis: {:.3e}, rate: {:.3f}".format(
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--num_layers", "-NL", type=int, default=2,
    ...
    parser.add_argument(
        ...
        choices=trc.IntegerDescreteFlows.nn_types,
    ...
    parser.add_argument(
        '--break_layer', type=int, default=0,
        help='early break')
    ...
    train_cmd.add_argument(
        "--reload_AE", action="store_true",
        help="Whether to reuse the AE weights")
