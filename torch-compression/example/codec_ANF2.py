    ...
    def evaluate(epoch, eval_lmda=None, is_resume=False):
        ...
        for idx, eval_imgs in enumerate(validate_dataloader):
            ...
            eval_imgs_tilde, eval_likelihoods, Y_error, jac, _, BDQ = coder(
                eval_imgs, visual=visual, figname=os.path.join(args.checkpoint_dir, "kodak_%02d" % (idx+1)))
            ...
            # bdq_psnr = psnr(eval_imgs, BDQ.mul(
            #     255).clamp(0, 255).round()).item()
            # print(bdq_psnr, eval_psnr.item(), eval_psnr.item()-bdq_psnr)
        ...
        with open(os.path.join(args.checkpoint_dir, "eval.txt"), 'w') as fp:
            ...
            for idx in range(len(logger)):
                ...
                line = "kodak_{idx:02d}:\t{psnr:.4f}\t{ssim:.4f}\t{rate:.4f}".format(
                ...
                fp.write(line+"\n")
            fp.write("AVG.\n")
            avg = "kodak:\t\t{psnr:.4f}\t{ssim:.4f}\t{rate:.4f}".format(
                **eval)
            for n in range(len(rates)):
