...
def train(args):
    ...
    coder = trc.hub.AugmentedNormalizedIntegerDiscreteFlowHyperPriorCoder(
    ...
    def evaluate(epoch, eval_lmda=None, is_resume=False):
        ...
        for idx, eval_imgs in enumerate(validate_dataloader):
            ...
            if args.enc_optim:
                ...
                #     for i in range(15):
                #         recompress, rell, _, _, _, _ = coder(recompress)
        ...
        if args.enc_optim or args.enc_noise or args.test_mode:
            exit(0)
        return eval['rd_loss']
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for iter, imgs in enumerate(pbar):
            imgs = imgs.to(DEVICE)
            imgs = imgs.mul(255).clamp(0, 255).round()
            ...
            imgs_tilde = trc.util.bound(imgs_tilde, 0, 255)
            disY = Y_error.pow(2).flatten(1).mean(1)
            if not args.cal_jac:
            ...
            if args.metric == 'PSNR':
                if args.use_huber:
                    distortion = huber_loss(imgs_tilde, imgs, 25)
                else:
                    distortion = (imgs - imgs_tilde).pow(2).flatten(1).mean(1)
                if torch.isnan(distortion).any() or distortion.gt(1e6).any():
                    print("dis", distortion)
                    print(imgs_tilde.min(), imgs_tilde.max())
                    flag = torch.isnan(distortion).any()
                ...
                if torch.isnan(rate).any():
                    print('rate', rate)
                    flag = True
                mask = distortion.lt(1e9)  # OPTIM
                if mask.sum().item() == 0:
                    rd_loss = distortion.mul(0).mean()
                else:
                    rd_loss = (args.lmda) * distortion + rate
                    if args.cal_yerr:
                        rd_loss += (args.lmda * 0.01) * disY
                    if args.cal_jac:
                        rd_loss -= (args.lmda * 1e-5) * jac.sum(1)
                    if args.rec_code:
                        rd_loss += code_err
        ...
        if epoch % (100 if args.dataset != "CLIC" else 500) == 0:
            torch.save({
                'epoch': epoch,
                'coder': coder.state_dict(),
                'optim': optim.state_dict()
...
def parse_args(argv):
    ...
    train_cmd.add_argument(
        "--valid_dataset", type=str, default='Kodak', choices=["Kodak", "CLIC2020_valid_pro", "CLIC2020_valid_mob", "Tecnick"],
        help="Valid dataset.")
    train_cmd.add_argument(
    ...
    train_cmd.add_argument(
        "--test_mode", "-TM", action="store_true",
        help="Test mode")
