...
from torch_compression.util.parameterizers import RDFTParameterizer
...
class SignalConv2d(Conv2d):
    ...
    def forward(self, input):
        # for torch>=1.7
        return self._conv_forward(input, self.weight if self.parameterizer is None else self.parameterizer(self.weight, self.kernel_size), self.bias)
        # for torch==1.4
