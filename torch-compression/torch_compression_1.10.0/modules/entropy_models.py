...
from torch_compression.util.quantization import (StochasticGumbelAnnealing,
                                                 noise_quant, quantize,
                                                 random_quant)
...
class BernoulliConditional(EntropyModel):
    """Conditional Bernoulli entropy model.

    This is a conditionally Bernoulli entropy model, analogous to
    `GaussianConditional`.
    """

    def __init__(self, **kwargs):
        super(BernoulliConditional, self).__init__(**kwargs)
        self._cdf_length = torch.Tensor([2]).short()
        self._offset = torch.Tensor([0]).short()
        self.condition_size = 1

    def _set_condition(self, condition):
        assert condition is not None, 'BernoulliConditional should given condition'
        self.mean, self.prob = None, condition.sigmoid()
    ...
    @torch.no_grad()
    def get_cdf(self, shape):
        B, C, H, W = self.prob.size()
        assert B == 1
        prob = self.prob.view(1, C, H, W, -1)

        pmf = torch.cat([1 - prob, prob], dim=-1)
        return ac.pmf2cdf(pmf), self._cdf_length.repeat(1, C, H, W), self._offset.to(self.prob.device)
    ...
    def quantize(self, input, mode, mean_holder):
        return input.gt(0.5).detach().float() - input.detach() + input

    def _likelihood(self, input):
        p_true = self.prob * input
        p_false = (1 - self.prob) * (1 - input)
        likelihood = torch.where(input.gt(0.5), p_true, p_false)

        return likelihood


class ConditionalFactorizer(nn.Module):
    """ConditionalFactorizer"""

    def __init__(self, num_features, condition, use_mean=True, K=5, quant_mode='noise'):
        super().__init__()
        self.num_features = num_features
        if "Mixture" not in condition:
            self.num_mixtures = 0
            self.condition = nn.Parameter(torch.Tensor(
                1, num_features*(2 if use_mean else 1), 1, 1))
            self.conditional_bottleneck = __CONDITIONS__[
                condition](use_mean=use_mean, quant_mode=quant_mode)
        else:
            self.num_mixtures = K
            self.condition = nn.Parameter(
                torch.Tensor(1, num_features*3*K, 1, 1))
            self.conditional_bottleneck = __CONDITIONS__[
                condition](K=K, use_mean=True, quant_mode=quant_mode)
        self.quantize = self.conditional_bottleneck.quantize

        self.reset_parameters()

    def reset_parameters(self):
        if not self.num_mixtures:
            mean, scale = self.condition.chunk(2, dim=1)
        else:
            mean, scale, pi = self.condition.chunk(3, dim=1)

        mean.data.zero_()
        if self.num_mixtures:
            pi.data.zero_()
            mean = mean.view(-1, self.num_features)
            for i in range(self.num_mixtures):
                mean.data[i] += i - (self.num_mixtures - 1) / 2.

        scale.data.fill_(1)

        # self.conditional_bottleneck._set_condition(self.condition)
        # print(self.conditional_bottleneck.mean[:, :, 0])
        # print(self.conditional_bottleneck.scale[:, :, 0])
        # print(self.conditional_bottleneck.pi[:, :, 0])
    ...
    def forward(self, input):
        self.conditional_bottleneck._set_condition(self.condition)
        # print('a', input.shape, self.conditional_bottleneck.quant_mode)
        output = self.quantize(input, self.conditional_bottleneck.quant_mode)
        likelihood = self.conditional_bottleneck._likelihood(output)
