            ...
            if code is None:
                ...
                if visual and code.size(1) == 3:
                    ...
                    visualizer.queue_visual(
                        loc.div(255.)+0.5 if self.integerlize else loc+0.5, figname+'_loc.png')
...
class SFTCondAugmentedNormalizedFlow(nn.Module):
    def __init__(self, use_code, distribution, gdn_mode,
                 transpose=False, clamp=1, integerlize=False):
        super(SFTCondAugmentedNormalizedFlow, self).__init__()
        self.use_code = use_code
        self.transpose = transpose
        self.distribution = distribution
        if distribution == 'gaussian':
    ...
    def get_condition(self, input, sft_cond, jac=None, layer=-1):
        prefix = self.name+("" if layer < 0 or str.isdigit(self.name[-1]) else "_"+str(layer)
                            )+"_"

        condition = self.net_forward(check_grad(prefix+'model_input', input), sft_cond)
    ...
    def forward(self, input, sft_cond, code=None, jac=None, rev=False, last_layer=False, layer=-1, visual=False, figname=''):
        if visual:
            debug(figname)
        if self.transpose:
            debug("T1")
