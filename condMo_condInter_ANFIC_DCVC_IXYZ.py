...
from models import Refinement, LocalAttentionBlock
...
def gaussian_p(x, mean, log_sd):
    return torch.exp(-0.5 * (x - mean) ** 2 / torch.exp(2 * log_sd)) / (torch.exp(log_sd) * sqrt(2 * pi))
...
class Pframe(CompressesModel):
    def __init__(self, args, mo_coder, cond_mo_coder, res_coder):
        ...
        self.frame_extractor = nn.Sequential(
            nn.Conv2d(3, self.args.hidden_channels, 3, stride=1, padding=1),
            ResBlock(self.args.hidden_channels, self.args.hidden_channels, 3),
        )
        ...
        # marginal_enc_x
        self.MCNet = nn.Sequential(
            ResBlock(self.args.hidden_channels, self.args.hidden_channels, 3),
            nn.Conv2d(self.args.hidden_channels, self.args.conditioning_channels, 3, stride=1, padding=1),
        )
        self.marginal_enc_y = nn.Sequential(
            ResBlock(self.args.hidden_channels, self.args.hidden_channels, 3),
            nn.Conv2d(self.args.hidden_channels, self.args.conditioning_channels, 3, stride=1, padding=1),
        )
        #self.joint_extractor = nn.Sequential(
        #    nn.Conv2d(3, self.args.hidden_channels, 3, stride=1, padding=1),
        #    ResBlock(self.args.hidden_channels, self.args.hidden_channels, 3),
        #)
        self.joint_attn = LocalAttentionBlock(self.args.hidden_channels)
    ...
    def motion_forward(self, ref_frame, coding_frame, visual=False, visual_prefix='', predict=False):
        ...
        if predict:
            ...
            warped = self.Resampler(self.frame_extractor(ref_frame), flow_hat)
            ...
            likelihoods = likelihood_m
            data = {'likelihood_m': likelihood_m, 'flow': flow, 'flow_hat': flow_hat, 'warped': warped}
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch < phase['trainMV']:
            ...
            for frame_idx in range(1, 3):
                ...
                #if frame_idx > 1:
                #   ref_frame = batch[:, frame_idx] - 1
                ref_frame = reconstructed#.detach()
                ...
                y_feat = self.frame_extractor(coding_frame)

                # p(z|x,y)
                p_z = self.joint_enc(self.joint_attn(data['warped'], y_feat))

                # q2(z|y)
                q2_z = self.marginal_enc_y(y_feat)

                # Sample z
                logsd = torch.zeros_like(p_z)
                sample_z = torch.randn_like(p_z) * torch.exp(logsd) + p_z # reparameterization
                
                # \hat p(z)
                batch_size, _, _, _, _ = batch.shape
                hat_p_z = torch.zeros_like(p_z)
                for bs in range(batch_size):
                    hat_p_z[bs : bs+1, :] = torch.sum(gaussian_p(sample_z[bs : bs+1, :], p_z, logsd), dim=0)

                hat_p_z /= batch_size
                
                # Calculate IXYZ
                # log q(z|x) + log q(z|y) - log p(z|x,y) - log p(z)
                IXYZ = (self.beta * gaussian_log_p(sample_z, mc, logsd) + \
                        self.beta * gaussian_log_p(sample_z, q2_z, logsd) - \
                        (2 * self.beta - 1) * gaussian_log_p(sample_z, p_z, logsd) -\
                        torch.log(hat_p_z))

                #reconstructed = reconstructed.clamp(0, 1)

                # Use gt frame as reference in this phase
                ...
                #loss += self.args.lmda * distortion.mean() + rate.mean() + 0.01 * self.args.lmda * mc_error.mean() - IXYZ.mean() * 0.1
                loss += self.args.lmda * distortion.mean() + rate.mean() - IXYZ.mean() * 0.1
            ...
            IXYZ = torch.mean(torch.tensor(IXYZ_list))
            ...
            logs = {
                    ...
                    'train/rd_loss': self.args.lmda * distortion.item() + rate.item(), 
                    'train/mc_error': mc_error.item(),
                    'train/IXYZ': IXYZ.item(),
            ...
            for frame_idx in range(1, 7):
                ...
                if epoch < phase['trainAll_IXYZ_fullgop'] and frame_idx > 4:
                    break
                ...
                if epoch < phase['trainAll_IXYZ_fullgop']:
                    y_feat = self.frame_extractor(coding_frame)

                    # p(z|x,y)
                    p_z = self.joint_enc(self.joint_attn(data['warped'], y_feat))
                    #joint_x_feat = self.Resampler(self.joint_extractor(ref_frame), data['flow_hat'])
                    #joint_y_feat = self.joint_extractor(coding_frame)
                    #p_z = self.joint_enc(self.joint_attn(joint_x_feat, joint_y_feat))
                    ...
                    #        (2 * self.beta - 1) * gaussian_log_p(sample_z, p_z, logsd) -\
                    #        torch.log(hat_p_z))
                    # Always set beta=1 here
                    IXYZ = (gaussian_log_p(sample_z, mc, logsd) + gaussian_log_p(sample_z, q2_z, logsd) - \
                            gaussian_log_p(sample_z, p_z, logsd) - torch.log(hat_p_z))
                    loss -= IXYZ.mean()
            ...
            if epoch < phase['trainAll_IXYZ_fullgop']:
                IXYZ = torch.mean(torch.tensor(IXYZ_list))
                logs.update({'train/IXYZ': IXYZ.item(),
                             'train/beta': self.beta})
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx, optimizer_closure=None, on_tpu=None,
        ...
        #if epoch >= phase['trainAll_2frames']:
        if epoch >= phase['trainMC']:
            self.beta = min(self.beta + 1/self.args.beta_annealing_steps, 1)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--beta_annealing_steps', default=10000, type=int)
        ...
        parser.add_argument('--beta_init', default=0., type=float)
...
if __name__ == '__main__':
    ...
    if args.restore == 'resume' or args.restore == 'finetune':
        ...
        for k, v in checkpoint['state_dict'].items():
            #if k.split('.')[0] == 'frame_extractor':
            #    key = '.'.join(['joint_extractor'] + k.split('.')[1:])
            #    new_ckpt[k] = v
            #elif k.split('.')[0] == 'MCNet':
            #    key = '.'.join(['joint_enc'] + k.split('.')[1:])
            #    new_ckpt[k] = v
            if k.split('.')[0] != 'frame_extractor' and k.split('.')[0] != 'MCNet':
                new_ckpt[k] = v
