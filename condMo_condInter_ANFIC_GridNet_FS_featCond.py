...
class Pframe(CompressesModel):
    ...
    def fuse_reference(self, intra_frame, previous_frame, flow_hat):
        ...
        #feats_out = []
        #cond_feat = fused_frame
        #for i, feature_extractor in enumerate(self.feature_extractors):
        #    _cond_feat = feature_extractor(cond_feat)
        #    feats_out.append(_cond_feat)
        #    cond_feat = _cond_feat
    ...
    def forward(self, intra_frame, previous_frame, coding_frame, p_order=0, visual=False, visual_prefix=''):
        ...
        reconstructed, likelihood_r, mc_hat, _, _, BDQ = self.Residual(coding_frame, output=mc, cond_coupling_input=fuse_info['feats_out'],
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            if epoch <= phase['trainRes_2frames_RecOnly']:
                ...
                reactivate_modules = [self.frame_synthesis, *self.feature_extractors]
            else:
            ...
            rec_frame, likelihood_r, mc_hat, _, _, BDQ = self.Residual(coding_frame, cond_coupling_input=fuse_info_1['feats_out'],
                                                                    output=mc_frame)
