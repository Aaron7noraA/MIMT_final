...
### Update ###
phase = {'trainMV': 5, 
         'trainRes_2frames_RecOnly': 9, 
         'trainRes_fullgop_RecOnly': 12,
         'trainRes_2frames': 15, 
         'trainRes_fullgop': 18,
         'trainAll_2frames': 24,
         'trainAll_fullgop': 34,
...
class Pframe(CompressesModel):
    def __init__(self, args, mo_coder, res_coder):
        ...
        self.MCNet = nn.Sequential(
            ResBlock(self.args.hidden_channels, self.args.hidden_channels, 3),
            nn.Conv2d(self.args.hidden_channels, self.args.hidden_channels, 3, stride=1, padding=1),
        )
    ...
    def forward(self, ref_frame, coding_frame, p_order=0, res_visual=False, visual_prefix = ''):
        ...
        if self.output_nought:
            ...
            reconstructed, likelihood_r, y2, _, _, BDQ = self.Residual(coding_frame, output=nought, cond_coupling_input=mc,
                                                                            visual=res_visual, figname=visual_prefix)
        else:
            raise NotImplementedError
            # reconstructed, likelihood_r, mc_hat, _, _, BDQ = self.Residual(coding_frame, output=decoded_mc, cond_coupling_input=mc,
            #                                                                 visual=res_visual, figname=visual_prefix)
        
        likelihoods = m_info['m_likelihood'] + likelihood_i + likelihood_r
        
        if self.output_nought:
            return reconstructed, likelihoods, m_info, m_info['flow_hat'], mc, predicted, intra_info, BDQ, y2
        else:
            raise NotImplementedError
        #     return reconstructed, likelihoods, m_info, m_info['flow_hat'], mc, predicted, intra_info, BDQ, mc_hat
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            ### Update ###
            if epoch <= phase['trainRes_fullgop_RecOnly']:
                _phase = '_RecOnly'
            ...
            for frame_idx in range(1, 7):
                ...
                ### Update ###
                if frame_idx > 1:
                    if epoch <= phase['trainRes_2frames_RecOnly'] or (phase['trainRes_fullgop_RecOnly'] < epoch and epoch <= phase['trainRes_2frames']) or (phase['trainRes_fullgop'] < epoch and epoch <= phase['trainAll_2frames']): # 2-frame training stages
                        break
                ...
                if _phase == 'RES':
                    for module in [self.MENet, self.Motion, self.MCNet]:
                ...
                if self.output_nought:
                    nought = torch.zeros_like(coding_frame)
                    reconstructed, likelihood_r, y2, _, _, _ = self.Residual(coding_frame, output=nought, cond_coupling_input=mc)
                else:
                    raise NotImplementedError
                #     reconstructed, likelihood_r, mc_hat, _, _, _ = self.Residual(coding_frame, output=decoded_mc, cond_coupling_input=mc)
                ...
                if self.output_nought:
                    ...
                    ### Update ###
                    if _phase == '_RecOnly':
                        loss += self.args.lmda * (distortion.mean() + 0.01 * y2_error.mean())
                    else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.output_nought:
            logs = {'dataset_name': dataset_name, 'seq_name': seq_name, 
                    ...
                    'val_m_rate': m_rate, 'val_y2_error': y2_error}
        else:
            raise NotImplementedError
            # logs = {'dataset_name': dataset_name, 'seq_name': seq_name, 
            #         'val_loss': loss, 'val_mse': mse, 
    ...
    def test_step(self, batch, batch_idx):
        metrics_name = ['PSNR', 'Rate', 'Mo_Rate', 'BDQ-PSNR', 'QE-PSNR', 'back-PSNR',
                        'p1-PSNR', 'p1-BDQ-PSNR']
        ...
        for frame_idx in range(gop_size):
            ...
            TO_VISUALIZE = (frame_id_start == 1 and frame_idx < 8 and seq_name in ['Beauty'])
            if frame_idx != 0:
                ...
                if False and TO_VISUALIZE:
                    ...
                    if self.output_nought:
                        ref_frame, likelihoods, _, flow_hat, mc_frame, _, _, BDQ, y2 = self(
...
if __name__ == '__main__':
    ...
    if args.restore == 'resume' or args.restore == 'finetune':
        ...
        ### Update ###
        checkpoint = torch.load("/home/u1481110/CANFVC+/DCVC_checkpoint.pth", map_location=(lambda storage, loc: storage))

        # for k, v in checkpoint['state_dict'].items():
        for k, v in checkpoint.items():
            if k[:15] == 'feature_extract':
                new_ckpt['frame_extractor' + k[15:]] = v
            if k[:14] == 'context_refine':
                new_ckpt['MCNet' + k[14:]] = v
