...
class BottleneckResidualBlock(nn.Sequential):
    """Builds the bottleneck residual block"""

    def __init__(self, num_filters, bias):
        super(BottleneckResidualBlock, self).__init__(
            nn.Conv2d(num_filters, num_filters//2, 1, stride=1, padding=0, bias=bias),
            nn.ReLU(inplace=True),
            nn.Conv2d(num_filters//2, num_filters//2, 3, stride=1, padding=1, bias=bias),
            nn.ReLU(inplace=True),
            nn.Conv2d(num_filters//2, num_filters, 1, stride=1, padding=0, bias=bias)
        )
...
class LocalAttentionBlock(nn.Module):
    """Builds the 'local' attention block"""

    def __init__(self, num_filters, bias=True):
        super(LocalAttentionBlock, self).__init__()
        self.trunk_branch = nn.Sequential(
            BottleneckResidualBlock(num_filters, bias),
            BottleneckResidualBlock(num_filters, bias),
            BottleneckResidualBlock(num_filters, bias)
        )
        ...
        nl = [
            ...
            BottleneckResidualBlock(num_filters, bias),
            BottleneckResidualBlock(num_filters, bias),
            nn.Conv2d(num_filters, num_filters, 1, stride=1, padding=0, bias=bias),
            nn.Sigmoid()
        ]
        self.attention_branch = nn.Sequential(*nl)

    def forward(self, input1, input2):
        return input1 + self.attention_branch(input2) * self.trunk_branch(input1)
