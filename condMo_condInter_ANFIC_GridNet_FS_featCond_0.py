...
class Pframe(CompressesModel):
    ...
    def fuse_reference(self, intra_frame, previous_frame, flow_hat):
        ...
        fused_frame, feats_out = self.frame_synthesis(feats)
    ...
    def forward(self, intra_frame, previous_frame, coding_frame, p_order=0, visual=False, visual_prefix=''):
        ...
        reconstructed, likelihood_r, mc_hat, _, _, BDQ = self.Residual(coding_frame, output=torch.zeros_like(previous_frame), 
                                                                        cond_coupling_input=fuse_info['feats_out'],
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            if epoch <= phase['trainRes_2frames']:
                frozen_modules = [self.MENet, self.Motion, self.MWNet, self.CondMotion]
                #frozen_modules = [self.Motion, self.MWNet, self.CondMotion]
                reactivate_modules = []
            else:
                frozen_modules = [self.MENet, self.MWNet]
                reactivate_modules = [self.Motion, self.CondMotion]
            ...
            rec_frame, likelihood_r, mc_hat, _, _, BDQ = self.Residual(coding_frame, cond_coupling_input=fuse_info['feats_out'],
                                                                       output=torch.zeros_like(I_frame))
            ...
            mc_error = nn.MSELoss(reduction='none')(torch.zeros_like(I_frame), mc_hat)

            if epoch <= phase['trainRes_2frames_RecOnly']:
                loss = self.args.lmda * distortion.mean() #+ 0.01 * self.args.lmda * mc_error.mean()
            else:
            ...
            rec_frame, likelihood_r, mc_hat, _, _, _ = self.Residual(coding_frame, cond_coupling_input=fuse_info_1['feats_out'],
                                                                     output=torch.zeros_like(I_frame))
            ...
            mc_error_1 = nn.MSELoss(reduction='none')(torch.zeros_like(I_frame), mc_hat)
            ...
            if epoch <= phase['trainRes_2frames_RecOnly']:
                loss += self.args.lmda * distortion_1.mean() #+ 0.01 * self.args.lmda * mc_error_1.mean()
            else:
            ...
            for frame_idx in range(1, 5):
                ...
                reconstructed, likelihood_r, mc_hat, _, _, _ = self.Residual(coding_frame, cond_coupling_input=fuse_info['feats_out'], 
                                                                             output=torch.zeros_like(I_frame))
                reconstructed = reconstructed.clamp(0, 1)
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                if frame_idx == 5:
                    ...
                    upload_img(BDQ.cpu().numpy()[0], f'{seq_name}_{epoch}_BDQ.png', grid=False)
...
if __name__ == '__main__':
    ...
    # **Force Residual coder to use standard conv**
    ...
    if args.restore == 'resume':
        ...
        #trainer.current_epoch = phase['trainMV'] - 2
        trainer.current_epoch = phase['trainMV'] + 1
