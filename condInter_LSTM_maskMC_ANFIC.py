...
class Pframe(CompressesModel):
    ...
    def motion_forward(self, ref_frame, coding_frame):
        ...
        mc, warped, mask = self.mc_net_forward(ref_frame, flow_hat)

        return mc, {'m_likelihood': likelihood_m, 'flow_hat': flow_hat, 'flow': flow, 'warped': warped, 'mask': mask, 'lstm_out': self.to_frame_h(self.hidden)}
    ...
    def mc_net_forward(self, ref_frame, coding_frame):
        ...
        if self.MCNet is not None:
            ...
            mask = torch.sigmoid(self.MCNet(*mc_net_input))

            mc_frame = mask * warped + (1.-mask) * self.to_frame_h(self.hidden)
        else:
            mc_frame = warped

        return mc_frame, warped, mask
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            flow = self.MENet(ref_frame, coding_frame)
            flow_hat, likelihood_m = self.Motion(flow)
            mc, _, _ = self.mc_net_forward(ref_frame, flow_hat)
            reconstructed = mc
            likelihoods = likelihood_m
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                rec_frame = align.resume(ref_frame)
    ...
    def test_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            ...
            if frame_idx != 0:
                ...
                mask = 1.-align.resume(m_info['mask'])
                mask = torch.cat([mask], dim=1)[0]
