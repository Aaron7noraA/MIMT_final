...
#phase = {'init': 0, 'trainMV': 10, 'trainMC': 20, 'trainRes_2frames': 50, 'trainALL_2frames': 80, 'trainALL_fullgop': 1000}
phase = {'init': 0, 'trainMV': 10, 'trainMC': 20, 'trainRes_2frames': 60, 'trainRes_fullgop':200, 'trainALL_fullgop': 1000}
...
class Pframe(CompressesModel):
    ...
    def forward(self, ref_frame, coding_frame, p_order=0, res_visual=False, visual_prefix = ''):
        ...
        res_hat, likelihood_r, Y_error, _, _, BDQ = self.Residual(res, visual=res_visual, figname=visual_prefix)
        reconstructed = mc + res_hat

        likelihoods = m_info['m_likelihood'] + likelihood_i + likelihood_r
        return reconstructed, likelihoods, None, m_info['flow_hat'], mc, predicted, intra_info, BDQ, Y_error
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            for frame_idx in range(1, 7):
                ...
                if epoch <= phase['trainRes_2frames'] and frame_idx > 1: # 2-frame training stages
                    break
                ...
                res = coding_frame - mc
                #res_hat, likelihood_r = self.Residual(res)
                res_hat, likelihood_r, Y_error, _, _, _ = self.Residual(res)
                ...
                distortion = self.criterion(coding_frame, reconstructed)
                rate = trc.estimate_bpp(likelihoods, input=coding_frame)
                #res_error = Y_error.pow(2).flatten(1).mean(1) * 255**2
                res_error = Y_error.pow(2).flatten(1).mean(1)

                loss += self.args.lmda * distortion.mean() + rate.mean() + 0.01 * self.args.lmda * res_error.mean()
                dist_list.append(distortion.mean())
                rate_list.append(rate.mean())
                res_error_list.append(res_error.mean())

            distortion = torch.mean(torch.tensor(dist_list))
            rate = torch.mean(torch.tensor(rate_list))
            res_error = torch.mean(torch.tensor(res_error_list))

            logs = {
                    'train/loss': loss.item() / 6, 
                    #'train/loss': loss.item() / 6, 
                    'train/distortion': distortion.item(), 
                    'train/PSNR': mse2psnr(distortion.item()), 
                    'train/rate': rate.item(), 
                    'train/Y_error': res_error.item()
                   }
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                rec_frame, likelihoods, _, flow_hat, mc_frame, _, _, BDQ, Y_error = self(align.align(ref_frame),
                ...
                Y_error = align.resume(Y_error).clamp(0, 1)
                ...
                if frame_idx == 5:
                    ...
                    upload_img(Y_error.cpu().numpy()[0], f'{seq_name}_predicted_MC_frame.png', grid=False)
    ...
    def test_step(self, batch, batch_idx):
        metrics_name = ['PSNR', 'Rate', 'Mo_Rate', 'MC-PSNR', 'BDQ-PSNR', 'QE-PSNR',
                        'p1-PSNR', 'p1-BDQ-PSNR']
        metrics = {}
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                if batch_idx % 100 == 0:
                    ...
                    ref_frame, likelihoods, _, flow_hat, mc_frame, _, _, BDQ, Y_error = self(
...
if __name__ == '__main__':
    ...
    # training specific
    parser.add_argument('--restore', type=str, choices=['none', 'load', 'load_from_DVC'], default='none')
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        period=1, # Save at least every 5 epochs
        verbose=True,
        monitor='val/loss',
        mode='min',
        prefix=''
    ...
    if args.restore == 'load':
        ...
        args_copy.architecture = "ANFHyperPriorCoder"
        args_copy.output_nought = True
        args_copy.cond_coupling = False
        ...
        model.Residual = trc.get_coder_from_args(args_copy)() # New residual coder
        ...
        checkpoint = torch.load(
            os.path.join(save_root, 'DVC_baseline', 'base_model_{}.ckpt'.format(args.lmda)),
            map_location=(lambda storage, loc: storage))
