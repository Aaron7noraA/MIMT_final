...
phase = {'init': 0, 'trainMV': 10, 'trainMC': 20, 'trainRes_2frames': 30, 'trainRes_fullgop': 50, 'trainALL_2frames': 60, 'trainALL_fullgop': 1000}
#phase = {'init': 0, 'trainMV': 20, 'trainMC': 40, 'trainRes_2frames': 68, 'trainALL_2frames': 98, 'trainALL_fullgop': 1000}
...
class Pframe(CompressesModel):
    ...
    def __init__(self, args, coder, ar_coder):
        ...
        #self.MCNet = Refinement(12, 64, out_channels=6) # channel # of Y:U:V=4:1:1 ; 2 inputs
        self.MCNet = Refinement(6, 64)

        #self.Residual = coder(in_channels=6, out_channels=6)
    ...
    def forward(self, ref_frame, coding_frame, p_order=0, res_visual=False, visual_prefix = ''):
        ...
        mc_YUV = RGB2YUV420(mc)
        coding_frame_Y, coding_frame_U, coding_frame_V = coding_frame
        coding_frame_YUV = torch.cat([space_to_depth(coding_frame_Y, block_size=2), coding_frame_U, coding_frame_V], dim=1)

        reconstructed, likelihood_r, mc_hat, _, _, _ = self.Residual(coding_frame_YUV, output=mc_YUV, cond_coupling_input=mc_YUV,
                                                                     visual=res_visual, figname=visual_prefix)

        return reconstructed, m_info['m_likelihood'], likelihood_r, m_info['flow_hat'], mc_YUV, mc_hat
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            ref_frame_RGB = YUV2RGB((ref_frame_Y, ref_frame_U, ref_frame_V), up=True)
            coding_frame_RGB = YUV2RGB((coding_frame_Y, coding_frame_U, coding_frame_V), up=True)

            flow = self.MENet(ref_frame_RGB, coding_frame_RGB)
            ...
            distortion = self.Criterion(reconstructed, coding_frame)
            ...
            flow_hat, likelihood_m = self.Motion(flow)
            reconstructed = self.Resampler(ref_frame, flow_hat)
            mc = self.mc_net_forward(ref_frame_RGB, flow_hat)
            reconstructed = mc
            ...
            for frame_idx in range(1, 7):
                ...
                if _phase == 'RES': # Train res_coder only
                    with torch.no_grad():
                        ...
                        warping_loss = self.criterion(mc, coding_frame_RGB)
                else:
                    flow = self.MENet(ref_frame_RGB, coding_frame_RGB)
                    flow_hat, likelihood_m = self.Motion(flow)
                    mc = self.mc_net_forward(ref_frame_RGB, flow_hat)
                ...
                rate = trc.estimate_bpp(likelihood_m, input=coding_frame) + trc.estimate_bpp(likelihood_r, input=coding_frame_yuv)
                mc_error = nn.MSELoss(reduction='none')(mc_YUV, mc_hat)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                rate = trc.estimate_bpp(likelihood_m, input=ref_frame).item() + \
                       trc.estimate_bpp(likelihood_r, input=ref_frame_Y).item()
                m_rate = trc.estimate_bpp(likelihood_m, input=ref_frame).item()
                ...
                size_byte = os.path.getsize(f'{dataset_root}/TestVideo/bpg/{qp}/bin_420/{seq_name}/frame_{frame_id}.bin')
    ...
    def test_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                ref_frame = align.align(ref_frame).clamp(0, 1)

                reconstructed_Y, reconstructed_U, reconstructed_V = (
                                                                     align.align(reconstructed_Y).clamp(0, 1), 
                                                                     align_UV.align(reconstructed_U).clamp(0, 1), 
                                                                     align_UV.align(reconstructed_V).clamp(0, 1)
                                                                    )
                reconstructed = align.align(reconstructed).clamp(0, 1)

                coding_frame_Y, coding_frame_U, coding_frame_V = (
                                                                  align.align(coding_frame_Y).clamp(0, 1), 
                                                                  align_UV.align(coding_frame_U).clamp(0, 1), 
                                                                  align_UV.align(coding_frame_V).clamp(0, 1)
                                                                 )
                coding_frame = align.align(coding_frame).clamp(0, 1)

                mc_frame_Y, mc_frame_U, mc_frame_V = (
                                                      align.align(mc_frame_Y).clamp(0, 1), 
                                                      align_UV.align(mc_frame_U).clamp(0, 1), 
                                                      align_UV.align(mc_frame_V).clamp(0, 1)
                                                     )
                
                mc_frame = align.align(mc_frame).clamp(0, 1)

                mc_hat_Y, mc_hat_U, mc_hat_V = (
                                                align.align(mc_hat_Y).clamp(0, 1), 
                                                align_UV.align(mc_hat_U).clamp(0, 1), 
                                                align_UV.align(mc_hat_V).clamp(0, 1)
                                               )
                mc_hat = align.align(mc_hat).clamp(0, 1)
                ...
                rate = trc.estimate_bpp(likelihood_m, input=reconstructed).item() + trc.estimate_bpp(likelihood_r, input=reconstructed_Y).item()
                m_rate = trc.estimate_bpp(likelihood_m, input=reconstructed).item()
