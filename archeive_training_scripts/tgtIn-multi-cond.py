...
#phase = {'init': 0, 'trainMV': 10, 'trainMC': 20, 'trainRes_2frames': 50, 'trainALL_2frames': 70,'trainRes_fullgop': 100, 'trainALL_fullgop': 1000}
phase = {'init': 0, 'trainMV': 10, 'trainMC': 20, 'trainRes_2frames': 50, 'trainRes_fullgop': 60, 'trainALL_fullgop': 1000}
...
class Pframe(CompressesModel):
    ...
    def forward(self, ref_frame, coding_frame, ref_queue, flow_queue, p_order=0, res_visual=False, visual_prefix = ''):
        ...
        reconstructed, likelihood_r, mmc_hat, _, _, BDQ = self.Residual(coding_frame, output=mmc, cond_coupling_input=mmc,
                                                                       visual=res_visual, figname=visual_prefix)

        likelihoods = m_info['m_likelihood'] + likelihood_i + likelihood_r
        return reconstructed, likelihoods, None, m_info['flow_hat'], mc, predicted, intra_info, BDQ, mmc_hat, mmc, ref_queue, flow_queue
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            flow = self.MENet(ref_frame, coding_frame)
            # flow_hat, likelihood_m, _ = self.Motion(flow, p_order=1)
            flow_hat, likelihood_m = self.Motion(flow)
            mc = self.mc_net_forward(ref_frame, flow_hat)
            ...
            for frame_idx in range(1, 7):
                ...
                # Residual coding
                reconstructed, likelihood_r, mmc_hat, _, _, _ = self.Residual(coding_frame, output=mmc, cond_coupling_input=mmc)
                
                likelihoods = likelihood_m + likelihood_r

                distortion = self.criterion(coding_frame, reconstructed)
                rate = trc.estimate_bpp(likelihoods, input=coding_frame)
                mmc_error = nn.MSELoss(reduction='none')(mmc, mmc_hat)

                loss += self.args.lmda * distortion.mean() + rate.mean() + 0.01 * self.args.lmda * mmc_error.mean()
                dist_list.append(distortion.mean())
                warping_loss_list.append(warping_loss.mean())
                rate_list.append(rate.mean())
                mmc_error_list.append(mmc_error.mean())
            ...
            mmc_error = torch.mean(torch.tensor(mmc_error_list))
            logs = {
                    'train/loss': loss.item() if epoch <= phase['trainRes_2frames'] else loss.item() / 6, # divide by 6 when RNN training stages
                    'train/distortion': distortion.item(), 
                    'train/PSNR': mse2psnr(distortion.item()), 
                    'train/rate': rate.item(), 
                    'train/mmc_error': mmc_error.item(),
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                rec_frame, likelihoods, _, flow_hat, mc_frame, _, _, BDQ, mmc_hat, mmc, ref_queue, flow_queue = self(align.align(ref_frame),
                ...
                mmc_hat = align.resume(mmc_hat).clamp(0, 1)
                ...
                if frame_idx == 5:
                    ...
                    upload_img(mmc_hat.cpu().numpy()[0], f'{seq_name}_predicted_MMC_frame.png', grid=False)
                ...
                psnr = mse2psnr(mse)
                mmc_mse = self.criterion(mmc, batch[:, frame_idx]).mean().item()
                mmc_psnr = mse2psnr(mmc_mse)
                mmc_error = nn.MSELoss(reduction='none')(mmc, mmc_hat).mean().item()
                loss = self.args.lmda * mse + rate + 0.01 * self.args.lmda * mmc_error

                mmc_psnr_list.append(mmc_psnr)
                m_rate_list.append(m_rate)

                # ref_frame = coding_frame
        ...
        mmc_psnr = np.mean(mmc_psnr_list)
        ...
        logs = {'seq_name': seq_name, 'val_loss': loss, 'val_mse': mse, 'val_psnr': psnr, 'val_rate': rate, 
                'val_mmc_psnr': mmc_psnr, 'val_m_rate': m_rate}
    ...
    def validation_epoch_end(self, outputs):
        ...
        uvg_rd = {'psnr': [], 'rate': [], 'mmc_psnr': [], 'm_rate': []}
        hevc_rd = {'psnr': [], 'rate': [], 'mmc_psnr': [], 'm_rate': []}
        loss = []
        ...
        for logs in [log['val_log'] for log in outputs]:
            ...
            if seq_name in dataset_name['UVG']:
                ...
                uvg_rd['mmc_psnr'].append(logs['val_mmc_psnr'])
                ...
                hevc_rd['mmc_psnr'].append(logs['val_mmc_psnr'])
        ...
        logs = {'val/loss': avg_loss,
                ...
                'val/UVG mmc_psnr': np.mean(uvg_rd['mmc_psnr']), 'val/UVG m_rate': np.mean(uvg_rd['m_rate']),
                'val/HEVC-B psnr': np.mean(hevc_rd['psnr']), 'val/HEVC-B rate': np.mean(hevc_rd['rate']),
                'val/HEVC-B mmc_psnr': np.mean(hevc_rd['mmc_psnr']), 'val/HEVC-B m_rate': np.mean(hevc_rd['m_rate']),
                }
        self.log_dict(logs)
    ...
    def test_step(self, batch, batch_idx):
        metrics_name = ['PSNR', 'Rate', 'Mo_Rate', 'MMC-PSNR', 'MMCrec-PSNR', 'MMCerr-PSNR', 'BDQ-PSNR', 'QE-PSNR', 'back-PSNR',
                        'p1-PSNR', 'p1-BDQ-PSNR', 'MC-PSNR']
        metrics = {}
        ...
        # MMCrec-PSNR: PSNR(gt, x_2)
        # MMCerr-PSNR: PSNR(x_2, mmc)
        # BDQ-PSNR: PSNR(gt, BDQ)
        # QE-PSNR: PSNR(BDQ, ADQ)
        # back-PSNR: PSNR(mmc, BDQ)
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                if batch_idx % 100 == 0:
                    ...
                    ref_frame, likelihoods, _, flow_hat, mc_frame, _, _, BDQ, mmc_hat, mmc, ref_queue, flow_queue = self(
                ...
                os.makedirs(self.args.save_dir + f'/{seq_name}/mmc_hat', exist_ok=True)
                ...
                if frame_id_start < 8 and seq_name in ['BasketballDrive', 'Kimono1', 'HoneyBee', 'Jockey']:
                    ...
                    save_image(BDQ[0], self.args.save_dir + f'/{seq_name}/BDQ/'
                               f'frame_{int(frame_id_start + frame_idx)}.png')
                    save_image(mmc_hat[0], self.args.save_dir + f'/{seq_name}/mmc_hat/'
                               f'frame_{int(frame_id_start + frame_idx)}.png')
                ...
                mmc_rec_psnr = mse2psnr(self.criterion(mmc_hat, coding_frame).mean().item())
                metrics['MMCrec-PSNR'].append(mmc_rec_psnr)
                
                mmc_err_psnr = mse2psnr(self.criterion(mmc, mmc_hat).mean().item())
                metrics['MMCerr-PSNR'].append(mmc_err_psnr)
                ...
                back_psnr = mse2psnr(self.criterion(mmc, BDQ).mean().item())
                ...
                log_list.append({'PSNR': psnr, 'Rate': rate, 'MMC-PSNR': mmc_psnr,
                                 ...
                                 'ry': estimate_bpp(likelihoods[2]).item(), 'rz': estimate_bpp(likelihoods[3]).item(),
                                  'MMCerr-PSNR': mmc_err_psnr, 'BDQ-PSNR': BDQ_psnr})
...
if __name__ == '__main__':
    ...
    # training specific
    parser.add_argument('--restore', type=str, choices=['none', 'resume', 'load', 'load_from_DVC', 'load_from_DVC_MMC'], default='none')
    ...
    if args.restore == 'resume':
        ...
        trainer.current_epoch = epoch_num + 2


        # Change res_coder
        import copy
        args_copy = copy.deepcopy(args)
        args_copy.architecture = "CondANFHyperPriorCoder"
    ...
    elif args.restore == 'load_from_DVC_MMC': # Custom for this training script ; since DVC_MMC is not under DVC_baseline
        comet_logger = CometLogger(
            api_key="bFaTNhLcuqjt1mavz02XPVwN8",
            project_name=project_name,
            workspace="tl32rodan",
