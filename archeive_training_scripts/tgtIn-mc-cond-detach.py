...
phase = {'init': 0, 'trainMV': 10, 'trainMC': 20, 'trainRes_2frames': 60, 'trainALL_2frames': 100, 'trainALL_fullgop': 1000}
...
class Pframe(CompressesModel):
    ...
    def forward(self, ref_frame, coding_frame, p_order=0, res_visual=False, visual_prefix = ''):
        ...
        #return reconstructed, likelihoods, None, m_info['flow_hat'], mc, predicted, intra_info, res, res_hat
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            for frame_idx in range(1, 7):
                ...
                #res = coding_frame - mc
                #res_hat, likelihood_r = self.Residual(res)
                reconstructed, likelihood_r, mc_hat, _, _, _ = self.Residual(coding_frame, output=mc, cond_coupling_input=mc.detach())
                                                                             #visual=True, figname='./debug_ANF_check_reverse/tmp')
