...
class Pframe(CompressesModel):
    ...
    def motion_forward(self, ref_frame, coding_frame):
        ...
        #ref_frame_yuv = torch.cat([
        #                            ref_frame_Y,    
        #                            F.interpolate(torch.cat([ref_frame_U, ref_frame_V], dim=1), scale_factor=2, mode='bilinear', align_corners=False)
        #                          ], dim=1)
        #coding_frame_yuv = torch.cat([
        #                                coding_frame_Y,    
        #                                F.interpolate(torch.cat([coding_frame_U, coding_frame_V], dim=1), scale_factor=2, mode='bilinear', align_corners=False)
        #                             ], dim=1)
        #
        #flow = self.MENet(ref_frame_yuv, coding_frame_yuv)
    ...
    def test_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                coding_frame_Y, coding_frame_U, coding_frame_V = (
                                                                  coding_frame_Y.clamp(0, 1), 
                                                                  coding_frame_U.clamp(0, 1), 
                                                                  coding_frame_V.clamp(0, 1)
                                                                 )
