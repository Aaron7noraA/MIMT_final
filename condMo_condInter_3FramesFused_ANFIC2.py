...
class Pframe(CompressesModel):
    def __init__(self, args, mo_coder, cond_mo_coder, res_coder):
        ...
        self.MCNet = Refinement(12, 64, out_channels=3)
    ...
    def motion_forward(self, ref_frame, coding_frame, visual=False, visual_prefix='', p_order=1):
        ...
        if predict:
            ...
            warped_frame_queue = self.warp_multiple_frames(frame_buffer, self.MWNet._flow_buffer + [flow_hat], True)
            self.MWNet.append_flow(flow_hat)
            
            mc_frame = self.MCNet(ref_frame, *warped_frame_queue)
            ...
            mc_frame = self.MCNet(ref_frame, warped_frame, warped_frame, warped_frame)
    ...
    def training_step(self, batch, batch_idx):
        ...
        #    }
            ...
            if epoch >= phase['trainAll_fullgop'] - 2: # Enable MENet's grad at the last epoch of fakeRNN
            ...
            for frame_idx in range(1, 6):
                ...
                with torch.set_grad_enabled(epoch >= phase['trainRes_2frames'] or epoch < phase['trainMC']):
                   mc, likelihood_m, data_1 = self.motion_forward(ref_frame, coding_frame, p_order=frame_idx)
                ...
                if epoch < phase['trainMC']:
                    likelihoods = likelihood_m
                    distortion = self.criterion(coding_frame, mc)
                else:
                    likelihoods = likelihood_m + likelihood_r
            ...
            if frame_idx != 0:
                ...
                if frame_idx < 3:
                    ...
                    if frame_idx == 2:
                        ...
                        flow_rgb = torch.from_numpy(
                            fz.convert_from_flow(flow_hat[0].permute(1, 2, 0).cpu().numpy()) / 255).permute(2, 0, 1)
                        upload_img(flow_rgb.cpu().numpy(), f'{seq_name}_{epoch}_{frame_idx}_pred_flow.png', grid=False)
                        
                        flow_hat = align.resume(m_info['pred_flow_hat'])
                        flow_rgb = torch.from_numpy(
                            fz.convert_from_flow(flow_hat[0].permute(1, 2, 0).cpu().numpy()) / 255).permute(2, 0, 1)
                        upload_img(flow_rgb.cpu().numpy(), f'{seq_name}_{epoch}_{frame_idx}_pred_flow_hat.png', grid=False)

                    upload_img(ref_frame.cpu().numpy()[0], f'{seq_name}_{epoch}_{frame_idx}_ref_frame.png', grid=False)
                    upload_img(coding_frame.cpu().numpy()[0], f'{seq_name}_{epoch}_{frame_idx}_gt_frame.png', grid=False)
                    upload_img(mc_frame.cpu().numpy()[0], seq_name + '_{:d}_{:d}_mc_frame_{:.3f}.png'.format(epoch, frame_idx, mc_psnr),
                               grid=False)
                    upload_img(rec_frame.cpu().numpy()[0], seq_name + '_{:d}_{:d}_rec_frame_{:.3f}.png'.format(epoch, frame_idx, psnr),
                               grid=False)

                if frame_idx < 5:
                    print(f'm_rate_{frame_idx} = {m_rate}')
                ref_frame = rec_frame
    ...
    def test_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            ref_frame = ref_frame.clamp(0, 1)
            #TO_VISUALIZE = frame_id_start == 1 and frame_idx < 8 and seq_name in ['Jockey', 'BQTerrace']
            TO_VISUALIZE = False
            if frame_idx != 0:
                coding_frame = batch[:, frame_idx]
                if frame_idx == 1:
                ...
                if TO_VISUALIZE:
                    ...
                    save_image(flow_map,
                               self.args.save_dir + f'/{seq_name}/flow/'
                                                    f'frame_{int(frame_id_start + frame_idx)}_origin_flow.png',
                               nrow=1)
                ...
                rate = trc.estimate_bpp(likelihoods, input=ref_frame).mean().item()
                #print('seq =', seq_name, '; frame_id =', int(frame_id_start + frame_idx), 'my =', estimate_bpp(likelihoods[0]).item(), 'mz =', estimate_bpp(likelihoods[1]).item())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore', type=str, choices=['none', 'resume', 'load', 'custom', 'onlyMC'], default='none')
