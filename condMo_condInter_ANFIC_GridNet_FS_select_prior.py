...
phase = {'trainMV': 7, 
         ...
         'trainAll_2frames': 16, 
         'trainAll_fullgop': 18, 
         'trainAll_prior_hp': 20, 
         'trainAll_prior_combine': 21, 
         'trainAll_prior_mix': 22, 
         'trainAll_RNN_1': 24, 
         'trainAll_RNN_2': 28, 
         'trainEM': 30,
         'train_aux': 100}
...
class Pframe(CompressesModel):
    ...
    def fuse_reference(self, intra_frame, previous_frame, flow_hat):
        ...
        prev_feat = previous_frame
        intra_feat = intra_feats[0]
        for i, feature_extractor in enumerate(self.feature_extractors):
            prev_feat = feature_extractor(prev_feat)
            prev_feats.append(self.Resampler(prev_feat, nn.functional.interpolate(flow_hat, scale_factor=2**(-i), mode='bilinear', align_corners=True) * 2**(-i)))

            intra_feat = feature_extractor(intra_feat)
            intra_feats.append(intra_feat)

        feats = [torch.cat([prev_feat, intra_feat], axis=1) for prev_feat, intra_feat in zip(prev_feats, intra_feats)]
    ...
    def motion_forward(self, ref_frame, coding_frame, visual=False, visual_prefix='', predict=False, use_prior='mix'):
        if predict:
    ...
    def forward(self, intra_frame, previous_frame, coding_frame, p_order=0, visual=False, visual_prefix='', use_prior='mix'):
        if p_order == 1:
            self.frame_buffer = [previous_frame]
            self.flow_buffer = list()
            likelihood_m, m_info = self.motion_forward(previous_frame, coding_frame, visual=visual, visual_prefix=visual_prefix, predict=False, use_prior=use_prior)
        else:
            likelihood_m, m_info = self.motion_forward(previous_frame, coding_frame, visual=visual, visual_prefix=visual_prefix, predict=True, use_prior=use_prior)
        ...
        reconstructed, likelihood_r, mc_hat, _, _, BDQ = self.Residual(coding_frame, output=mc, cond_coupling_input=mc,
                                                                        visual=visual, figname=visual_prefix, use_prior=use_prior)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            frozen_modules = [self.Motion, self.CondMotion, self.MWNet] 
            for module in frozen_modules:
                for param in module.parameters(): 
                        self.optimizers().state[param] = {} # remove all state (step, exp_avg, exp_avg_sg)
            ...
            if epoch < phase['trainAll_prior_mix']:
                self.feature_extractors.requires_grad_(False)
                self.frame_synthesis.requires_grad_(False)
                self.Motion.requires_grad_(False)
                self.CondMotion.analysis0.requires_grad_(False)
                self.CondMotion.analysis1.requires_grad_(False)
                self.CondMotion.synthesis0.requires_grad_(False)
                self.CondMotion.synthesis1.requires_grad_(False)
                self.CondMotion.DQ.requires_grad_(False)
            ...
            if epoch < phase['trainAll_prior_hp']:
                use_prior = 'hp'
                self.CondMotion.pred_prior.requires_grad_(False)
                self.CondMotion.PA.requires_grad_(False)
                self.CondMotion.mask_gen.requires_grad_(False)
                self.Residual.pred_prior.requires_grad_(False)
                self.Residual.PA.requires_grad_(False)
                self.Residual.mask_gen.requires_grad_(False)
            elif epoch < phase['trainAll_prior_combine']:
                use_prior = 'combine'
                self.CondMotion.hyper_analysis.requires_grad_(False)
                self.CondMotion.hyper_synthesis.requires_grad_(False)
                self.Residual.hyper_analysis.requires_grad_(False)
                self.Residual.hyper_synthesis.requires_grad_(False)
            else:
                use_prior = 'mix'
            ...
            for frame_idx in range(1, 6):
                ...
                if frame_idx == 1:
                    self.frame_buffer = [previous_frame]
                    likelihood_m, m_info = self.motion_forward(previous_frame, coding_frame, predict=False, use_prior=use_prior)
                else:
                    likelihood_m, m_info = self.motion_forward(previous_frame, coding_frame, predict=True, use_prior=use_prior)
                ...
                reconstructed, likelihood_r, mc_hat, _, _, _ = self.Residual(coding_frame, cond_coupling_input=mc_frame, 
                                                                             output=mc_frame, use_prior=use_prior)
                reconstructed = reconstructed.clamp(0, 1)
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                = self(align.align(I_frame),
                       ...
                       p_order=frame_idx, use_prior=use_prior)
                ...
                previous_frame = align.resume(previous_frame).clamp(0, 1)
                ...
                m_rate = trc.estimate_bpp(likelihoods[:2], input=previous_frame).mean().item()
    ...
    def test_step(self, batch, batch_idx):
        ...
        I_frame = batch[:, 0]  # Put reference frame in first position
        previous_frame = I_frame
            ...
            if frame_idx != 0:
                ...
                if TO_VISUALIZE:
                    ...
                    if frame_idx != 1 and args.compute_macs:
                        ...
                        macs, params = get_model_complexity_info(self, tuple(align.align(previous_frame).shape), input_constructor=dummy_inputs)
                ...
                previous_frame = rec_frame.detach()
                previous_frame = align.resume(previous_frame)
                ...
                m_rate = trc.estimate_bpp(likelihoods[0], input=previous_frame).mean().item() + \
                         trc.estimate_bpp(likelihoods[1], input=previous_frame).mean().item()
                ...
                QE_psnr = mse2psnr(self.criterion(BDQ, previous_frame).mean().item())
    ...
    def setup(self, stage):
        ...
        if stage == 'fit':
            ...
            # self.test_dataset = VideoTestDataIframe(dataset_root, self.args.lmda, sequence=('U', 'B', 'M', 'K'), GOP=32)
            # self.test_dataset = VideoTestDataIframe(dataset_root, self.args.lmda, sequence=('U', 'B'), GOP=32)
            self.test_dataset = VideoTestDataIframe(dataset_root, self.args.lmda, sequence=('U', 'B'), GOP=args.gop)
            # self.test_dataset = VideoTestDataIframe(dataset_root, self.args.lmda, sequence=('B'), GOP=self.args.gop)
