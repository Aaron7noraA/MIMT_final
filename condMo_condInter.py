...
class Pframe(CompressesModel):
    ...
    def motion_forward(self, ref_frame, coding_frame, visual=False, visual_prefix='', predict=False):
        ...
        if predict:
            ...
            data = {'likelihood_m': likelihood_m, 'flow_hat': flow_hat, 'mc_frame': mc_frame, 'pred_frame': pred_frame, 'pred_flow': pred_flow, 'pred_flow_hat': pred_flow_hat, 'warped_frame': warped_frame}
            ...
            likelihoods = likelihood_m
            data = {'likelihood_m': likelihood_m, 'flow_hat': flow_hat, 'mc_frame': mc_frame, 'warped_frame': warped_frame}
    ...
    def forward(self, ref_frame, coding_frame, p_order=0, visual=False, visual_prefix='', predict=False):
        mc, likelihood_m, m_info = self.motion_forward(ref_frame, coding_frame, visual=visual, figname=visual_prefix, predict=predict)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                size_byte = os.path.getsize(f'{dataset_root}/TestVideo/bpg_crop/{qp}/bin/{seq_name}/frame_{frame_id}.bin')
    ...
    def setup(self, stage):
        ...
        if stage == 'fit':
            ...
            self.test_dataset = VideoTestDataIframe(dataset_root, self.args.lmda, sequence=('D'))
