...
class Pframe(CompressesModel):
    ...
    def forward(self, ref_frame, coding_frame, prev_y1=None ,p_order=0, res_visual=False, visual_prefix = ''):
        mc, m_info = self.motion_forward(ref_frame, coding_frame)
        prev_y1 = self.Resampler(prev_y1, m_info['flow_hat'])

        predicted, intra_info, likelihood_i = mc, 0, ()
        
        if self.output_nought:
            nought = torch.zeros_like(coding_frame)
            reconstructed, likelihood_r, y2, _, _, BDQ, decoded = self.Residual(coding_frame, output=nought, cond_coupling_input=[mc, prev_y1], pred_prior_input=mc,
                                                                            visual=res_visual, figname=visual_prefix)
        else:
            reconstructed, likelihood_r, mc_hat, _, _, BDQ, decoded = self.Residual(coding_frame, output=mc, cond_coupling_input=[mc, prev_y1], pred_prior_input=mc,
                                                                            visual=res_visual, figname=visual_prefix)
        ...
        if self.output_nought:
            return reconstructed, likelihoods, m_info, m_info['flow_hat'], mc, predicted, intra_info, BDQ, y2, y1
        else:
            return reconstructed, likelihoods, m_info, m_info['flow_hat'], mc, predicted, intra_info, BDQ, mc_hat, y1
            ...
            for frame_idx in range(1, 7):
                ...
                with torch.set_grad_enabled( _phase == 'ALL'): 
                    ...
                    if frame_idx == 1:
                        ...
                        prev_y1 = decoded[1]
                        prev_y1 = self.Resampler(prev_y1, flow_hat)
                
                if self.output_nought:
                    nought = torch.zeros_like(coding_frame)
                    reconstructed, likelihood_r, y2, _, _, _, decoded = self.Residual(coding_frame, output=nought, 
                                                                            cond_coupling_input=[mc, prev_y1], pred_prior_input=mc,)
                else:
                    reconstructed, likelihood_r, mc_hat, _, _, _, decoded = self.Residual(coding_frame, output=mc,
                                                                            cond_coupling_input=[mc, prev_y1], pred_prior_input=mc,)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                if self.output_nought:
                    rec_frame, likelihoods, m_info, flow_hat, mc_frame, _, _, BDQ, y2, y1 = self(align.align(ref_frame),
                                                                                      align.align(batch[:, frame_idx]),
                                                                                      prev_y1=y1 if frame_idx > 1 else mc_frame/2,
                                                                                      p_order=frame_idx)
                else:
                    rec_frame, likelihoods, m_info, flow_hat, mc_frame, _, _, BDQ, mc_hat, y1 = self(align.align(ref_frame),
                                                                                      align.align(batch[:, frame_idx]),
                                                                                      prev_y1=y1 if frame_idx > 1 else mc_frame/2,
                                                                                      p_order=frame_idx)
                ...
                if frame_idx == 1:
                    y1 = align.resume(mc_frame/2).clamp(0, 1)
                else:
                    y1 = align.resume(y1).clamp(0, 1)
                ...
                if frame_idx == 2:
                    ...
                    upload_img(y1.cpu().numpy()[0], f'{seq_name}_y1.png', grid=False)
                    ...
                    upload_img(rec_frame.cpu().numpy()[0],
                               seq_name + '_rec_frame_{:.3f}_{}.png'.format(mc_psnr, frame_idx),
                               grid=False)
        ...
        for frame_idx in range(gop_size):
            ...
            if frame_idx != 0:
                ...
                if False and TO_VISUALIZE:
                    ...
                    if self.output_nought:
                        ref_frame, likelihoods, _, flow_hat, mc_frame, _, _, BDQ, y2, y1 = self(
                        ...
                        ref_frame, likelihoods, _, flow_hat, mc_frame, _, _, BDQ, mc_hat, y1 = self(
                                                                                                  align.align(ref_frame),
                                                                                                  align.align(batch[:, frame_idx]),
                                                                                                  p_order=frame_idx,
                                                                                                  prev_y1=y1 if frame_idx > 1 else mc_frame/2,
                ...
                if TO_VISUALIZE: 
                    ...
                    save_image(mc_frame[0], self.args.save_dir + f'/{seq_name}/mc_frame/'
                               f'frame_{int(frame_id_start + frame_idx)}.png')
                    save_image(y1[0], self.args.save_dir + f'/{seq_name}/mc_frame/'
                               f'y1_{int(frame_id_start + frame_idx)}.png')
