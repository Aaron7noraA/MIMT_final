...
class Pframe(CompressesModel):
    ...
    def forward(self, ref_frame, coding_frame, prev_y1=None, p_order=0, visual=False, visual_prefix='', predict=False):
        ...
        refined_y1, _ = self.motion_compensation(prev_y1, m_info['flow_hat'], self.feature_extractors_y, self.MCNet_y)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch < phase['trainMV']:
            ...
            self.MWNet.requires_grad_(False)
            if self.args.restore == 'finetune' or self.args.restore == 'resume':
                self.requires_grad_(True)
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                rec_frame, likelihood_r, mc_hat, _, _, BDQ, decoded = self.Residual(align.align(coding_frame), output=None,
                                                                        cond_coupling_input=[mc_frame, prev_y1], pred_prior_input=mc_frame,)
            ...
            if frame_idx != 0:
                ...
                if frame_idx == 1:
                    ...
                    mc_frame, likelihood_m, data = self.motion_forward(align.align(ref_frame),
                                                                       align.align(coding_frame), predict=True)
                    prev_y1 = decoded[0]
                    prev_y1, _ = self.motion_compensation(prev_y1, data['flow_hat'], self.feature_extractors, self.MCNet_y)

                rec_frame, likelihood_r, mc_hat, _, _, BDQ, decoded = self.Residual(align.align(coding_frame), output=None,
                                                                           cond_coupling_input=[mc_frame, prev_y1], 
