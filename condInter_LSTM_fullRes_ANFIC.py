...
class Pframe(CompressesModel):
    def __init__(self, args, mo_coder, res_coder):
        ...
        self.if_extractor = nn.Sequential(
                                nn.Conv2d(3, self.args.hidden_channels*2, kernel_size=5, stride=1),
                                nn.LeakyReLU(0.01)
                           )
        ...
        self.convLSTM = ConvLSTMCell(9, self.args.hidden_channels, kernel_size=(5, 5), stride=1, bias=True)
        self.hidden = None
        self.cell = None
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            flow = self.MENet(ref_frame, coding_frame)
            flow_hat, likelihood_m = self.Motion(flow)
            mc, _ = self.mc_net_forward(ref_frame, flow_hat)
            reconstructed = mc
            likelihoods = likelihood_m
...
if __name__ == '__main__':
    ...
    if args.restore == 'resume' or args.restore == 'finetune':
        ...
        for k, v in checkpoint['state_dict'].items():
            if k[:5] != 'MCNet' and k.split('.')[0] != 'convLSTM' and k.split('.')[0] != 'if_extractor': 
                new_ckpt[k] = v
