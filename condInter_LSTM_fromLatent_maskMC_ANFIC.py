...
from torch_compression.hub import AugmentedNormalizedFlowHyperPriorCoder, ConvTranspose2d, GeneralizedDivisiveNorm
...
class Pframe(CompressesModel):
    def __init__(self, args, mo_coder, res_coder):
        ...
        self.if_extractor = nn.Sequential(
                                nn.Conv2d(3, self.args.hidden_channels*2, kernel_size=5, stride=1, padding=2),
                                nn.LeakyReLU(0.01)
                           )
        ...
        self.MaskNet = Refinement(3 + self.args.hidden_channels, 64, out_channels=1)
        self.MCNet = Refinement(6, 64, out_channels=3)

        self.Residual = res_coder

        # Hidden state channel # = 8
        self.convLSTM = ConvLSTMCell(3+self.args.hidden_channels, self.args.hidden_channels, kernel_size=(5, 5), stride=1, bias=True)
        self.hidden = None
        self.cell = None
        
        res_coder_cfg = yaml.safe_load(open(self.args.residual_coder_conf, 'r'))
        # z2 will be decoded for LSTM updating with shape=(h, w, self.args.hidden_channels)
        self.z2_decoder = nn.Sequential(
                                ConvTranspose2d(res_coder_cfg['model_params']['num_features'], 32, 5, stride=2),
                                GeneralizedDivisiveNorm(32, inverse=True, simplify=False),
                                ConvTranspose2d(32, 32, 5, stride=2),
                                GeneralizedDivisiveNorm(32, inverse=True, simplify=False),
                                ConvTranspose2d(32, 32, 5, stride=2),
                                GeneralizedDivisiveNorm(32, inverse=True, simplify=False),
                                ConvTranspose2d(32, self.args.hidden_channels, 5, stride=2),
                          )
        self.hidden_to_frame = nn.Sequential(
                                nn.Conv2d(self.args.hidden_channels, 3, kernel_size=5, stride=1, padding=2),
                                nn.LeakyReLU(0.01)
                           )
    ...
    def motion_forward(self, ref_frame, coding_frame, fix_mask=False):
        ...
        mc, warped, mask, fused_frame, buffer_image = self.mc_net_forward(ref_frame, flow_hat, fix_mask)

        return mc, {'m_likelihood': likelihood_m, 'flow_hat': flow_hat, 'flow': flow, 'warped': warped, 
                    'mask': mask, 'fused_frame': fused_frame, "buffer_image": buffer_image}

    def mc_net_forward(self, ref_frame, coding_frame, fix_mask=False):
        warped = self.Resampler(ref_frame, coding_frame)

        if not self.args.zeroMo_LSTM:
            self.hidden = self.Resampler(self.hidden, coding_frame)
            self.cell = self.Resampler(self.cell, coding_frame)

        if self.MCNet is None:
            raise ValueError
        if fix_mask:
            mask = torch.ones_like(ref_frame[:, :1, :, :])*0.5
            #mask = torch.zeros_like(ref_frame[:, :1, :, :])
        else:
            mask = torch.sigmoid(self.MaskNet(warped, self.hidden))
        buffer_image = self.hidden_to_frame(self.hidden)
        
        fused_frame = mask * warped + (1.-mask) * buffer_image

        mc_frame = self.MCNet(ref_frame, fused_frame)

        return mc_frame, warped, mask, fused_frame, buffer_image

    def forward(self, ref_frame, coding_frame, p_order=0, res_visual=False, visual_prefix = '', fix_mask=False):
        mc, m_info = self.motion_forward(ref_frame, coding_frame, fix_mask)

        reconstructed, likelihood_r, mc_hat, _, z_2, BDQ, _ = self.Residual(coding_frame, output=mc, cond_coupling_input=mc,
                                                                                        visual=res_visual, figname=visual_prefix)
        
        # Update convLSTM
        decoded_signals = self.z2_decoder(z_2)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            #flow = self.MENet(ref_frame, coding_frame)
            #flow_hat, likelihood_m = self.Motion(flow)
            #mc, _, _, _, _ = self.mc_net_forward(ref_frame, flow_hat, fix_mask=True)
            #reconstructed = mc
            #likelihoods = likelihood_m
            ...
            for frame_idx in range(1, 7):
                ...
                mc, m_info = self.motion_forward(ref_frame, coding_frame, fix_mask=True)
                
                buffer_image = m_info['buffer_image']

                with torch.set_grad_enabled(False): 
                    _, _, _, _, z_2, _, _ = self.Residual(coding_frame, output=mc, cond_coupling_input=mc)
                # Update convLSTM
                decoded_signals = torch.zeros_like(self.z2_decoder(z_2))
                self.hidden, self.cell = self.convLSTM(torch.cat([decoded_signals, mc], dim=1), (self.hidden, self.cell))

                reconstructed = mc
                likelihoods = m_info['m_likelihood']
                
                distortion = self.criterion(coding_frame, reconstructed) + self.criterion(coding_frame, buffer_image)
                rate = trc.estimate_bpp(likelihoods, input=coding_frame)
                ...
                # Use ground truth as reference in this step
                reconstructed = coding_frame
            ...
            if epoch == phase['trainMask']:
                self.requires_grad_(False)
                self.MaskNet.requires_grad_(True)
            else:
            ...
            for frame_idx in range(1, 7):
                ...
                with torch.set_grad_enabled( _phase == 'ALL'): 
                    mc, m_info = self.motion_forward(ref_frame, coding_frame, fix_mask=(epoch < phase['trainMask']))
                
                with torch.set_grad_enabled(epoch != phase['trainMask']): 
                    reconstructed, likelihood_r, mc_hat, _, z_2, _, _ = self.Residual(coding_frame, output=mc, cond_coupling_input=mc)
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                rec_frame, likelihoods, m_info, flow_hat, mc_frame, _, _, BDQ, mc_hat = self(align.align(ref_frame),
                                                                                 ...
                                                                                 p_order=frame_idx, fix_mask=(self.current_epoch <= phase['trainMask']))
                ...
                if frame_idx == 1:
                    ...
                    mask = m_info['mask']
                    mask = torch.cat([mask]*3, dim=1)[0].cpu().numpy()
                    upload_img(mask, f'{seq_name}_mask.png', grid=False)
                ...
                self.hidden, self.cell = self.if_extractor(rec_frame).split(self.args.hidden_channels, dim=1)
            ...
            if frame_idx != 0:
                ...
                if False: #TO_VISUALIZE:
                    ...
                    ref_frame, likelihoods, m_info, flow_hat, mc_frame, _, _, BDQ, mc_hat = self(
                                                                                             ...
                                                                                             fix_mask=(self.current_epoch <= phase['trainMask'])
                                                                                           )
                else:
                    #continue 
                ...
                mc_hat = align.resume(mc_hat).clamp(0, 1)
                BDQ = align.resume(BDQ).clamp(0, 1)
                warped = align.resume(m_info['warped']).clamp(0, 1)
                buffer_image = align.resume(m_info['buffer_image']).clamp(0, 1)
                fused_frame = align.resume(m_info['fused_frame']).clamp(0, 1)
                ...
                if TO_VISUALIZE:
                    ...
                    save_image(flow_map, self.args.save_dir + f'/{seq_name}/flow/f{int(frame_idx)}_origin_flow.png', nrow=1)

                    save_image(coding_frame[0], self.args.save_dir + f'/{seq_name}/gt_frame/'
                               f'frame_{int(frame_id_start + frame_idx)}.png')
                    save_image(warped[0], self.args.save_dir + f'/{seq_name}/mc_frame/'
                               f'frame_{int(frame_id_start + frame_idx)}_warped.png')
                    save_image(buffer_image[0], self.args.save_dir + f'/{seq_name}/mc_frame/'
                               f'frame_{int(frame_id_start + frame_idx)}_buffer_image.png')
                    save_image(fused_frame[0], self.args.save_dir + f'/{seq_name}/mc_frame/'
                               f'frame_{int(frame_id_start + frame_idx)}_fused_frame.png')
                    save_image(mc_frame[0], self.args.save_dir + f'/{seq_name}/mc_frame/'
                               f'frame_{int(frame_id_start + frame_idx)}.png')
                    mask = m_info['mask']
                    mask = torch.cat([mask]*3, dim=1)[0]
                    save_image(mask, self.args.save_dir + f'/{seq_name}/mc_frame/'
                               f'frame_{int(frame_id_start + frame_idx)}_mask.png')
    ...
    def compress(self, ref_frame, coding_frame, p_order):
        ...
        mc_frame, _ = self.mc_net_forward(ref_frame, flow_hat)
    ...
    def setup(self, stage):
        ...
        qp = {128: 37, 256: 37, 512: 32, 1024: 27, 2048: 22, 4096: 22}[self.args.lmda]
        ...
        if stage == 'fit':
            ...
            self.test_dataset = VideoTestDataIframe(dataset_root, self.args.lmda, sequence=('U'), GOP=32)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--lmda', default=2048, choices=[128, 256, 512, 1024, 2048, 4096], type=int)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--zeroMo_LSTM', action="store_true", help='No warping before using the LSTM output')
    ...
    if args.ssim:
        ...
        ANFIC_code = {2048: '0821_0300', 1024: '0530_1212', 512: '0530_1213', 256: '0530_1215', 128: '0530_1215'}[args.lmda]
    ...
    if args.restore == 'resume' or args.restore == 'finetune':
        ...
        for k, v in checkpoint['state_dict'].items():
            if k.split('.')[0] != 'MCNet' and k.split('.')[0] != 'MENet' and k.split('.')[0] != 'convLSTM' and k.split('.')[0] != 'upsampler_h' and k.split('.')[0] != 'if_extractor': 
                new_ckpt[k] = v
        model = Pframe(args, mo_coder, res_coder).cuda()
        ...
        #                        map_location=(lambda storage, loc: storage))
        #spy_ckpt = torch.load(os.path.join(save_root, "ANF-based-resCoder-for-DVC", "91c98b563d4d4a8d96fb866cf593698b", "checkpoints", "epoch=38.ckpt"),
        #                        map_location=(lambda storage, loc: storage))
        ...
        #for k, v in spy_ckpt['state_dict'].items():
        #    if k.split('.')[0] == 'MENet':
