...
class Pframe(CompressesModel):
    ...
    def motion_compensation(self, ref_frame, flow):
        ...
        if self.args.MCNet == 'UNet':
            ...
            for f in feat:
                warped_feat.append(self.Resampler(f, flow))
                flow = F.interpolate(flow, scale_factor=0.5, mode='bilinear', align_corners=False)//2
    ...
    def forward(self, ref_frame, coding_frame, prev_y1=None, p_order=0, visual=False, visual_prefix='', predict=False):
        ...
        reconstructed, likelihood_r, mc_hat, _, _, BDQ, decoded = self.Residual(coding_frame, output=mc, cond_coupling_input=[prev_y1, mc], pred_prior_input=mc,
        ...
        return reconstructed, likelihoods, None, m_info['flow_hat'], mc, predicted, intra_info, BDQ, mc_hat, y1
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch < phase['trainMV']:
            ...
            rec_frame, likelihood_r, mc_hat, _, _, _, _ = self.Residual(coding_frame, output=mc,
                                                                        cond_coupling_input=[prev_y1, mc], pred_prior_input=mc,)
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                rec_frame, likelihood_r, mc_hat, _, _, BDQ, decoded = self.Residual(align.align(coding_frame), output=mc_frame,
                                                                        cond_coupling_input=[prev_y1, mc_frame], pred_prior_input=mc_frame,)
    ...
    def test_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            ...
            TO_VISUALIZE = frame_id_start == 1 and (frame_idx == 1 or frame_idx == 8) and seq_name in ['BasketballDrive', 'Jockey']
            if frame_idx != 0:
                coding_frame = batch[:, frame_idx]
                ...
                if TO_VISUALIZE:
                    ...
                    rec_frame, likelihood_r, mc_hat, _, _, BDQ, decoded = self.Residual(align.align(coding_frame), output=mc_frame,
                                                                               cond_coupling_input=[prev_y1, mc_frame], 
