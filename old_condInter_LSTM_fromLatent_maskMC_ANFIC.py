...
from models import Refinement_feat
...
class Pframe(CompressesModel):
    def __init__(self, args, mo_coder, res_coder):
        ...
        self.MCNet = Refinement_feat(6 + self.args.hidden_channels, 64, out_channels=1, feat_channels=self.args.hidden_channels)

        self.Residual = res_coder

        # Hidden state channel # = 8
        self.convLSTM = ConvLSTMCell(2*self.args.hidden_channels, self.args.hidden_channels, kernel_size=(5, 5), stride=1, bias=True)
        self.hidden = None
        self.cell = None
        self.lstm_output = None
        
        res_coder_cfg = yaml.safe_load(open(self.args.residual_coder_conf, 'r'))
        # z2 will be decoded for LSTM updating with shape=(h/2, w/2, self.args.hidden_channels)
        ...
        self.upsampler_h = nn.Sequential(
                                ConvTranspose2d(self.args.hidden_channels, self.args.hidden_channels, kernel_size=5, stride=2),
                                nn.LeakyReLU(0.01)
                           )
        self.to_frame_o = nn.Sequential(
    ...
    def motion_forward(self, ref_frame, coding_frame):
        ...
        mc, mc_feat, warped, mask = self.mc_net_forward(ref_frame, flow_hat)

        return mc, mc_feat, {'m_likelihood': likelihood_m, 'flow_hat': flow_hat, 'flow': flow, 'warped': warped, 'mask': mask, 'lstm_out': self.to_frame_o(self.lstm_output)}

    def mc_net_forward(self, ref_frame, coding_frame):
        warped = self.Resampler(ref_frame, coding_frame)

        self.lstm_output = self.upsampler_h(self.hidden)

        if not self.args.zeroMo_LSTM:
            self.lstm_output = self.Resampler(self.lstm_output, coding_frame)
            self.hidden = self.Resampler(self.hidden, F.interpolate(coding_frame, scale_factor=0.5, mode='bilinear', align_corners=False)//2)
        ...
        if self.MCNet is None:
            raise ValueError
        mc_net_input = [ref_frame, warped, self.lstm_output]
        
        mask, mc_feat = self.MCNet(*mc_net_input)
        mask = torch.sigmoid(mask)
        
        mc_frame = mask * warped + (1.-mask) * self.to_frame_o(self.lstm_output)

        return mc_frame, mc_feat, warped, mask

    def forward(self, ref_frame, coding_frame, p_order=0, res_visual=False, visual_prefix = ''):
        mc, mc_feat, m_info = self.motion_forward(ref_frame, coding_frame)
        ...
        self.hidden, self.cell = self.convLSTM(torch.cat([decoded_signals, mc_feat], dim=1), (self.hidden, self.cell))
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            flow = self.MENet(ref_frame, coding_frame)
            flow_hat, likelihood_m = self.Motion(flow)
            mc, mc_feat, _, _ = self.mc_net_forward(ref_frame, flow_hat)
            reconstructed = mc
            likelihoods = likelihood_m
        ...
        for frame_idx in range(gop_size):
            ...
            if frame_idx != 0:
                ...
                if TO_VISUALIZE:
                    ...
                    lstm_output = m_info['lstm_out'][0]
                    save_image(lstm_output, self.args.save_dir + f'/{seq_name}/mc_frame/'
                               f'frame_{int(frame_id_start + frame_idx)}_lstm_output.png')
