...
class Pframe(CompressesModel):
    def __init__(self, args, mo_coder, cond_mo_coder, res_coder):
        ...
        self.if_model = AugmentedNormalizedFlowHyperPriorCoder(128, 320, 192, num_layers=2, use_DQ=True, use_code=False,
                                                               ...
                                                               quant_mode='RUN', get_decoded=True)
    ...
    def motion_forward(self, ref_frame, coding_frame, visual=False, visual_prefix='', predict=False):
        ...
        if predict:
            ...
            data = {'likelihood_m': likelihood_m, 
                    'flow': flow, 'flow_hat': flow_hat, 'warped_frame': warped_frame,
            ...
            likelihoods = likelihood_m
            data = {'likelihood_m': likelihood_m, 'flow': flow, 'flow_hat': flow_hat, 'warped_frame': warped_frame,}
    ...
    def training_step(self, batch, batch_idx):
        ...
        # I-frame
        with torch.no_grad():
            ref_frame, _, _, _, _, _, decoded = self.if_model(ref_frame)
        ...
        if epoch < phase['trainMV']:
            ...
            for frame_idx in range(1, 7):
                ...
                with torch.set_grad_enabled(epoch < phase['trainRes_2frames']): 
                    ...
                    prev_y1 = decoded[0]
                    prev_y1 = self.Resampler(prev_y1, data_1['flow_hat'])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            if frame_idx != 0:
                ...
                prev_y1 = decoded[0]
                prev_y1 = self.Resampler(prev_y1, data['flow_hat'])
                ...
                with torch.no_grad():
                    rec_frame, likelihoods, _, _, _, _, decoded = self.if_model(align.align(batch[:, frame_idx]))
