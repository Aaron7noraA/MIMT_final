...
import yaml
from dataloader import VimeoDataset, VideoTestDataBPGIframe
...
phase = {'trainMV': -1, 
         ...
         'trainRes_2frames': -1, 
         'trainAll_2frames': -1, 
         'trainAll_fullgop_hp': 20, 
         'trainAll_fullgop_tp': 50, 
         #
         'trainAll_2frames_hp': 15, 
         'trainAll_2frames_tp': 35, 
         #
         'trainAll_fullgop': 100, 
         'trainAll_RNN_1': 110, 
         'trainAll_RNN_2': 120, 
         #'trainEM': 70,
         'train_aux': 100}
...
class Pframe(CompressesModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if epoch <= phase['trainMV']:
            ...
            num_frames = 1 if epoch <= phase['trainAll_2frames'] or epoch <= phase['trainAll_2frames_hp'] or \
                             (epoch <= phase['trainAll_2frames_tp'] and epoch > phase['trainAll_fullgop_hp'] ) \
                           else 4

            for frame_idx in range(1, num_frames+1):
                self.Residual.conditional_bottleneck.training_mask_ratio = random.uniform(0., min(1, epoch/phase['trainAll_fullgop']))
                ...
                if epoch <= phase['trainAll_fullgop_hp']:
                    self.Residual.use_prior = 'hp'
                elif epoch <= phase['trainAll_fullgop_tp']:
                    self.Residual.use_prior = 'tp'
                else:    
                    self.Residual.use_prior = 'combine'

                reconstructed, likelihood_r, mc_hat, _, _, BDQ = self.Residual(coding_frame, 
                                                                               cond_coupling_input=mc_frame, 
                                                                               output=mc_frame, 
                                                                               train_rate_only=(epoch <= phase['trainAll_fullgop']),
                                                                               return_mask=True,
                                                                               #visual=True, figname='./tmp/frame_'+str(frame_idx)
                                                                              )
                reconstructed = reconstructed.clamp(0, 1)
                ...
                if epoch <= phase['trainAll_fullgop_tp']:
                    rate_ry = trc.estimate_bpp(likelihood_r[0], input=coding_frame)
                else:
                    likelihood_r, mask = likelihood_r
                    #rate_ry = trc.estimate_bpp(likelihood_r[0], input=coding_frame, mask=mask)
                    rate_ry = trc.estimate_bpp(likelihood_r[0], input=coding_frame)

                rate_rz = trc.estimate_bpp(likelihood_r[1], input=coding_frame)
                if epoch <= phase['trainAll_fullgop']:
                    distortion = distortion.detach()
                    rate_m = rate_m.detach()
                    if epoch > phase['trainAll_fullgop_hp']:
                        rate_rz = rate_rz.detach()

                rate_r = rate_ry + rate_rz
                rate = rate_m + rate_r 
            ...
            logs = {
                    ...
                    'train/rate_m': rate_m.item(), 
                    'train/rate_r': rate_r.item(), 
    ...
    def test_step(self, batch, batch_idx):
        ...
        for frame_idx in range(gop_size):
            ...
            TO_VISUALIZE = False and frame_id_start == 1 and frame_idx < 8 and seq_name in ['BasketballDrive', 'Kimono1', 'Jockey', 'HoneyBee']
            ...
            if frame_idx != 0:
                ...
                r_rate = trc.estimate_bpp(likelihoods[2], input=previous_frame).mean().item() + \
                         trc.estimate_bpp(likelihoods[3], input=previous_frame).mean().item()
    ...
    def setup(self, stage):
        ...
        if stage == 'fit':
            ...
            self.train_dataset = VimeoDataset(dataset_root + "/vimeo_septuplet/", 'BPG_QP' + str(qp), 7,
                                                 transform=transformer)
            self.val_dataset = VideoTestDataBPGIframe(dataset_root, self.args.lmda, first_gop=True)

        elif stage == 'test':
            self.test_dataset = VideoTestDataBPGIframe(dataset_root, self.args.lmda, sequence=('U', 'B'), GOP=args.gop)
...
if __name__ == '__main__':
    ...
    if args.restore == 'resume':
        ...
        trainer.current_epoch = phase['trainAll_fullgop_tp'] + 1
        #trainer.current_epoch = epoch_num + 1
        ...
        #    elif 'Decoder' in k and 'blocks' in k:
        #        if 's_attn' in k:
        #            key = k.replace('s_attn', 's_attn.attn')
        #            new_ckpt[key] = v

        #        elif 'c_attn' in k:
        #            key = k.replace('c_attn', 'c_attn.attn')
        #            new_ckpt[key] = v
        ...
        #        else:
        #            key = '.'.join(k.split('.')[:6] + ['s_attn'] + k.split('.')[6:])
        #            new_ckpt[key] = v
        #            key = '.'.join(k.split('.')[:6] + ['c_attn'] + k.split('.')[6:])
        #            new_ckpt[key] = v
        ...
        #summary(model.Residual.conditional_bottleneck.MIMT_encoder)
        #summary(model.Residual.conditional_bottleneck.MIMT_decoder)
        #print(model.Residual.conditional_bottleneck.MIMT_encoder)
        #print(model.Residual.conditional_bottleneck.MIMT_decoder)
